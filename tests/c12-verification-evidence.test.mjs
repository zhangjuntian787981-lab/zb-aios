...
const evidencePath =
  "implementation/p1/c12/c12-verification-evidence.v1.json";
...
test("C12 verification evidence is source-bound and Synthetic-only", async () => {
  const evidence = await json(evidencePath);
  assert.equal(evidence.work_package_id, "C12");
  assert.equal(evidence.evidence_ref, evidencePath);
  assert.equal(
    evidence.verified_source_commit,
    "3f0de9af5edbfcf3912065ea1e486fffe6f728c5",
  );
  ...
  assert.equal(evidence.review.p2_findings, 1);
  assert.equal(evidence.source_artifacts.count, 12);
  assert.equal(
  ...
  const matrix = await json(
    "implementation/p1/c12/verification-matrix.v1.json",
  );
