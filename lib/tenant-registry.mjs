...
export function createTenantRegistry({
  ...
  async function isSnapshotAuthorized(context, id) {
    if (
      typeof context?.actorId !== "string" ||
      !context.actorId.trim() ||
      (
        Object.hasOwn(context, "tenantId") &&
        context.tenantId !== id
      )
    ) {
      return false;
    ...
    try {
      return (await authorize(context, READ, request)) === true;
    } catch {
      return false;
    }
  }
  ...
  async function snapshot(context, id) {
    tenantId(id);
    if (!(await isSnapshotAuthorized(context, id))) {
