...
const AUTHORIZATION_PROMPT = Object.freeze({
	...
	byteLength: 16491,
	sha256: "sha256:a7e5d4b8334ae0cac5a6d9c0bf90c40dc9150c4a1bad1be1e2c1675c6facceaf",
});
...
export const OPENAI_TERRA_MAX_MODEL_VISIBLE_BYTES = 650 * 1024;
...
function jsonSection(section, label) {
	if (!section) throw new TypeError(`${label} is missing from the Review Subject.`);
	const text = new TextDecoder("utf-8", { fatal: true }).decode(section.bytes);
	const value = JSON.parse(text);
	if (text !== canonicalize(value) && text !== `${JSON.stringify(value, null, 2)}\n`) {
