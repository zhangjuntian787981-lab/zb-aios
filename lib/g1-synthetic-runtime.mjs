...
function secretFactory() {
  ...
  return () => {
    call += 1;
    return call.toString(16).padStart(64, "0");
  };
}
...
async function createPrincipalPlatform(identityPlatform) {
...
async function createAuthorizationPlatform(identityPlatform, principalPlatform) {
...
async function createModelPlatform(
  ...
  function gateway(catalogDocument, idPrefix) {
  ...
  return {
    gateway: gateway(draftCatalogDocument, "4000"),
    reviewGateway: gateway(reviewCatalogDocument, "4100"),
    provider,
  };
}
...
async function createToolPlatform(
  ...
  const gateway = createToolGateway({
    ...
    catalog,
    store: createMemoryToolGatewayStore({ clock: () => NOW }),
    credentialBroker,
    adapter,
    clock: () => NOW,
...
async function createDecisionPlatform(
  ...
  const store = createMemoryHumanDecisionStore({ clock: () => NOW });
...
async function createOrchestrationPlatform({
  ...
  function instanceFor(tenant) {
    ...
    const orchestrator = createAgentOrchestrator({
      ...
      statePort: createMemoryC12StatePort(),
      catalog,
      policyGate: {
...
async function startC08Run({
  ...
  const receipts = createC08C0MockToolReceipts();
...
async function finishC08Run(state, tenant, c12Result) {
  ...
  await state.core.recordToolCallResult(
    {
      synthetic: true,
  ...
  await state.execute(`g1-c08-finish-${tenantSuffix}`, {
    kind: "FINISH_RUN",
    runId: state.started.runId,
...
function createAuditPlatform(identityPlatform, principalPlatform) {
...
function createObservabilityPlatform(identityPlatform) {
    ...
    async record({
      ...
      const replayBinding = {
        ...
        toolReservationId:
          toolReservation.reservation.reservationId,
        modelReceiptRef:
          modelSettlement.settlement.receiptRef,
        toolReceiptRef: toolSettlement.settlement.receiptRef,
        totalSignals: telemetry.sli.totalSignals,
        bookedCostMicros: cost.totals.bookedCostMicros,
      };
      const prior = completed.get(tenant.tenantId);
      if (
        prior &&
        observabilitySha256(prior) !==
          observabilitySha256(replayBinding)
      ) {
        throw new Error("G1 C19 replay changed its evidence.");
      }
      if (!prior) completed.set(tenant.tenantId, replayBinding);
      return { ...result, replayed: Boolean(prior) };
    },
  };
}
...
export async function createG1SyntheticRuntime() {
  const identityPlatform = await createIdentityPlatform();
  const principalPlatform = await createPrincipalPlatform(identityPlatform);
  ...
  const observabilityPlatform =
    createObservabilityPlatform(identityPlatform);
