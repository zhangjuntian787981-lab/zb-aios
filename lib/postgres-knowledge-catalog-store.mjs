...
function storageEffectFromRow(row) {
  if (!row) return null;
  return Object.freeze({
    tenantId: row.tenant_id,
    tenantKind: row.tenant_kind,
...
export function createPostgresKnowledgeCatalogStore({
  ...
  async function prepareStorageEffectOnce(scope, envelope) {
    return runScoped(
      runtimePool,
      "aios_c10_runtime",
      scope,
      ...
      async (client) => {
        ...
        const stored = await client.query(
          `SELECT *
             FROM ${table("storage_effect")}
            WHERE tenant_id=$1 AND idempotency_key=$2
            FOR UPDATE`,
          [scope.tenantId, envelope.idempotencyKey],
        );
        if (stored.rowCount === 1) {
          const pendingEffect = storageEffectFromRow(stored.rows[0]);
          if (
            pendingEffect.requestHash !== envelope.requestHash ||
            pendingEffect.effectKind !== envelope.effectKind
          ) {
            fail(
              "IDEMPOTENCY_CONFLICT",
          ...
          if (pendingEffect.status !== "PENDING") {
            fail(
              "INTEGRITY_VIOLATION",
        ...
        await writeNodes(client, current, next, internalType);
        await client.query(
          `INSERT INTO ${table("storage_effect")} (
             tenant_id,tenant_kind,idempotency_key,request_hash,effect_kind,
             status,document_id,document_version,quarantine_ref,
             content_sha256,command,created_at,completed_at
           ) VALUES (
             $1,$2,$3,$4,$5,'PENDING',$6,$7,$8,$9,$10::jsonb,$11,NULL
           )`,
          [
            next.tenantId,
            next.tenantKind,
  ...
  async function prepareStorageEffect(scope, envelope) {
    validateScope(scope);
    if (
    ...
    for (let attempt = 0; ; attempt += 1) {
      try {
        return await prepareStorageEffectOnce(scope, envelope);
      } catch (error) {
        const retryable =
          RETRYABLE_SQL_STATES.has(error?.code) ||
          (error?.code === "23505" &&
            error?.constraint === "storage_effect_pkey");
        if (!retryable || attempt >= maxSerializableRetries) {
          throw databaseFailure(error);
        }
        await new Promise((resolve) =>
  ...
  async function completeStorageEffectOnce(scope, envelope) {
    return runScoped(
      runtimePool,
      "aios_c10_runtime",
      scope,
      ...
      async (client) => {
        ...
        if (completed.rowCount !== 1) {
          fail(
            "INTEGRITY_VIOLATION",
            "Pending storage effect was not completed.",
          );
        }
        return result;
      },
  ...
  async function completeStorageEffect(scope, envelope) {
    validateScope(scope);
    for (let attempt = 0; ; attempt += 1) {
      try {
        return await completeStorageEffectOnce(scope, envelope);
      } catch (error) {
        const retryable =
          RETRYABLE_SQL_STATES.has(error?.code) ||
          (error?.code === "23505" &&
  ...
  async function listPendingStorageEffects(scope, { limit = 100 } = {}) {
    try {
      return await runScoped(
        ...
        async (client) => {
          ...
          return Object.freeze(result.rows.map(storageEffectFromRow));
        },
      );
    } catch (error) {
      throw databaseFailure(error);
  ...
  async function readReceipt(scope, request) {
    try {
      return await runScoped(
        runtimePool,
        "aios_c10_runtime",
        ...
        async (client) => {
          ...
          if (receipt.rows[0].request_hash !== request.requestHash) {
            fail(
              "IDEMPOTENCY_CONFLICT",
              "Idempotency key was reused with another request.",
            );
                 ...
                 SELECT DISTINCT ON (revision.document_version)
                  ...
                  WHERE revision.tenant_id=$1
                    ...
                    AND NOT (
                      ...
                      AND document.updated_at <= $3::timestamptz
                    )
                    ${version}
