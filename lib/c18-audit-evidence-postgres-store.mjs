...
function validateLeaseReceipt(input, failure = false) {
  ...
  if (failure) {
    ...
    if (Date.parse(input.retryAt) <= Date.parse(input.now)) {
      fail("INVALID_INPUT", "retryAt is invalid.");
    }
  }
}
...
function databaseFailure(error) {
  ...
  if (
    ...
    error?.constraint === "audit_outbox_metadata_only"
  ) {
...
export function createPostgresAuditEvidenceStore({
  ...
  if (
    ...
    new Set([writerPool, readerPool, outboxPool, scopePool]).size !== 4 ||
          ...
          async (client) => {
            ...
            await client.query(
              `INSERT INTO ${table("audit_outbox")} (
                 tenant_id,tenant_kind,event_id,event,status,
                 attempt_count,lease_version,leased_by,lease_until,
                 available_at,published_at,last_error_code,created_at
               ) VALUES (
                 $1,'SYNTHETIC',$2,$3::jsonb,'PENDING',
                 0,0,NULL,NULL,$4::timestamptz,NULL,NULL,$4::timestamptz
               )`,
              [
                record.tenantId,
                record.eventId,
      ...
      return await runScoped(
        ...
        async (client) => {
          ...
          return {
            ...
            tenantId: trusted.tenantId,
            tenantKind: "SYNTHETIC",
            events: result.rows.map(eventFromRow),
          };
        },
      );
    } catch (error) {
