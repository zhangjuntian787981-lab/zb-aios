import frozenAttestation from "../implementation/p2/attestations/p2-execution-baseline-attestation.bc718b.v1.json" with { type: "json" };
import {
...
function bindingMatchesAttestation(binding, attestation, startPolicy) {
  if (
    !exactKeys(binding, [
    ...
    !exactValue(binding.policy, startPolicy) ||
...
export function createP2WorkerAttestationVerifier({
  attestation,
  pinPolicy,
  startPolicy = P2_V2_CANDIDATE_START_POLICY,
}) {
  let assets = null;
  try {
    assets = deepFreeze({
      ...
      startPolicy: structuredClone(startPolicy),
    });
  } catch {
    assets = null;
  }
