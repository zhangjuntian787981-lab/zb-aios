...
export async function runG1PersistentIsolationMatrix(options) {
  ...
  for (const surface of SURFACE_NAMES) {
    ...
    for (
      ...
      for (
        ...
        const owner = tenant.users[ownerIndex];
        const currentResourceId = resourceId(surface, owner);
  ...
  const persistenceSatisfied = surfaceResults.every(
    ({ persistent }) => persistent === true,
  );
  const provenanceSatisfied = surfaceResults.every(
    ({ surface, observations }) =>
      observations.backendKind ===
      EXPECTED_BACKEND_KIND_BY_SURFACE[surface],
  );
  const remainingGaps = [];
  ...
  return {
    ...
    assuranceLimit:
      "Declared backend kinds require source-backed integration verification.",
    caseClassSemantics: {
