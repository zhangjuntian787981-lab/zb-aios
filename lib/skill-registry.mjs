...
export function createSkillRegistry({
  ...
  async function execute(serverContext, request) {
    ...
    if (!RESOURCE_ID.test(resources?.manageResourceId ?? "")) {
      fail("INVALID_CONFIGURATION", "C13 MANAGE resource is invalid.");
    }
    const trusted = await trustedInputs(
      serverContext,
      request,
      "MANAGE",
      resources.manageResourceId,
      `C13_${request.command.kind}`,
    );
    const metadata = {
  ...
  async function resolveForRun(serverContext, query) {
    ...
    if (!RESOURCE_ID.test(resources?.readResourceId ?? "")) {
...
function validateEvaluationReport(report) {
  if (
    ...
    !Number.isSafeInteger(report.caseCount) ||
    report.caseCount < 1 ||
    ...
    typeof report.reportRef !== "string" ||
    ...
    fail("EVALUATION_REPORT_INVALID", "Synthetic evaluation report is invalid.");
  }
}
