...
const SYSTEM_TOOLCHAIN = Object.freeze({
  shasumExecutable: "/usr/bin/shasum",
  perlExecutable: "/usr/bin/perl",
});
...
async function captureGitToolchainBinding() {
  ...
  const xcrunCachePath = cachePathStdout.trim();
  if (!xcrunCachePath.startsWith("/")) {
    throw new TypeError("Independent review xcrun cache path is invalid.");
  }
  const { stdout: versionStdout } = await execFileAsync(
    GIT_TOOLCHAIN.shimExecutable,
    ["--version"],
    {
      encoding: "utf8",
      env: {
  ...
  const [
    ...
    executableBinding(GIT_TOOLCHAIN.xcrunExecutable),
    executableBinding(xcrunCachePath),
  ]);
  const binding = {
...
async function captureSystemToolchainBinding() {
  const [shasumExecutable, perlExecutable] = await Promise.all([
    executableBinding(SYSTEM_TOOLCHAIN.shasumExecutable),
    executableBinding(SYSTEM_TOOLCHAIN.perlExecutable),
  ]);
  const binding = {
    shasumExecutable,
    perlExecutable,
...
async function resolveFrozenDependencyRoot(dependencyRoot) {
  ...
  const packageRoots = await Promise.all(
    PACKAGE_NAMES.map((name) => realpath(join(exactInputRoot, name))),
  );
  ...
  if (
    packageRoots.some(
      (packageRoot, index) =>
        ...
        packageRoot !== resolve(exactContentRoot, PACKAGE_NAMES[index]),
    )
  ) {
    throw new TypeError(
      "Runtime dependency packages do not share one frozen root.",
    );
  }
...
export async function captureIndependentReviewRuntimeBinding({
  ...
  const exactDependencyRoot =
    await resolveFrozenDependencyRoot(dependencyRoot);
  ...
  const systemToolchain = await captureSystemToolchainBinding();
  const binding = {
...
export function validateIndependentReviewRuntimeBinding(binding) {
  ...
  const validGitToolchain =
    ...
    validToolchainFile(binding.gitToolchain.xcrunExecutable) &&
    validToolchainFile(binding.gitToolchain.xcrunCache) &&
  ...
  const validSystemToolchain =
    exactKeys(binding?.systemToolchain, [
      "shasumExecutable",
      "perlExecutable",
      "bindingSha256",
    ]) &&
    validToolchainFile(binding.systemToolchain.shasumExecutable) &&
    validToolchainFile(binding.systemToolchain.perlExecutable) &&
    SHA256.test(binding.systemToolchain.bindingSha256 ?? "") &&
    binding.systemToolchain.bindingSha256 ===
      sha256Value(
        Object.fromEntries(
          Object.entries(binding.systemToolchain).filter(
            ([key]) => key !== "bindingSha256",
          ),
        ),
      );
