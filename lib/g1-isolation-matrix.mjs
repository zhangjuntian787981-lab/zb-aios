...
function createRoleBoundGate(fixtures, runtime) {
  ...
  return Object.freeze({
    ...
    observations() {
      return {
        realC04SessionCount: fixtures.reduce(
          (count, fixture) => count + fixture.users.length,
          0,
        ),
        realC05PrincipalCount: new Set(
          fixtures.flatMap((fixture) =>
            fixture.users.map(({ principalId }) => principalId)
          ),
        ).size,
        realC06AllowCount: allowCount,
        realC06DenyCount: denyCount,
...
export async function runG1IsolationMatrix() {
  ...
  try {
    ...
    return {
      ...
      gateConditionStatus: exhaustive ? "SATISFIED" : "NOT_SATISFIED",
      exhaustive,
      observedLeakCount,
      ...
      remainingGaps: exhaustive
        ? []
        : ["G1_MATRIX_NOT_EXHAUSTIVE_OR_ZERO_LEAK"],
