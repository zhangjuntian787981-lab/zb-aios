...
function projectState(compiled, journalState) {
  const packageState = new Map(
    compiled.packages.map((item) => [
      item.id,
      {
        ...
        evidenceRefs: [...(item.artifact_refs ?? [])],
        evidenceHashes: [...(item.evidence_hashes ?? [])],
        verifiedAt: item.verified_at ?? null,
        updatedAt: item.verified_at ?? compiled.manifest.generated_at,
      },
    ]),
  );
  const submissions = [];
  ...
  const phaseEntry = {
    ...
    P2: approvedGates.has("G1"),
    P3: approvedGates.has("G2"),
  };

  const workPackages = compiled.packages.map((item) => {
    const state = packageState.get(item.id);
    const blockers = [];
    if (item.phase_entry_gate && !approvedGates.has(item.phase_entry_gate)) {
      blockers.push(item.phase_entry_gate);
    }
    for (const dependency of item.depends_on ?? []) {
...
export function createProjectControl({
  ...
  async function snapshot() {
    return projectState(compiled, await journal.load(compiled.manifest.project_id));
  }
  ...
  async function execute(context, command) {
    ...
    const current = projectState(compiled, journalState);
    let event;
