...
function validateAuthorizationEvidence(
  ...
  if (
    ...
    !Number.isSafeInteger(evidence?.humanSecurityEpoch) ||
    evidence.humanSecurityEpoch < 1 ||
    evidence?.workloadActorPrincipalId !== workloadActorPrincipalId ||
    !PRINCIPAL_ID.test(evidence?.workloadActorPrincipalId ?? "") ||
    !Number.isSafeInteger(evidence?.workloadActorSecurityEpoch) ||
    evidence.workloadActorSecurityEpoch < 1 ||
    evidence?.leafDelegationId !== leafDelegationId ||
    !DELEGATION_ID.test(evidence?.leafDelegationId ?? "") ||
    !SHA256.test(evidence?.delegationChainSha256 ?? "") ||
    typeof evidence?.purposeRef !== "string" ||
    !evidence.purposeRef ||
...
function projectionNext(current, command, tenantEpoch) {
  ...
  return Object.freeze({
    ...
    validFrom:
      command.state === "PUBLISHED"
        ? command.metadata?.validFrom ?? null
        : null,
    validUntil:
      command.state === "PUBLISHED"
        ? command.metadata?.validUntil ?? null
        : null,
    classification:
      command.state === "PUBLISHED"
        ? command.metadata?.classification ?? null
        : null,
    acl:
      command.state === "PUBLISHED"
        ? clone(command.metadata?.acl ?? null)
        : null,
  ...
  return Object.freeze({
    ...
    async search(scope, filter, input) {
      ...
      const observedIndexEpoch =
        tenantEpochs.get(scope.tenantId) ?? 0;
      const candidates = [];
    ...
    async readCache(scope, filter, cacheKey) {
      validateScope(scope);
      validateTrustedFilter(filter);
      const scopedCacheKey = `${scope.tenantId}\u0000${cacheKey}`;
      const cache = caches.get(scopedCacheKey);
      ...
      if (
        ...
        Date.parse(cache.asOf) > Date.parse(filter.asOf) ||
        Date.parse(cache.expiresAt) <= Date.parse(filter.asOf)
      ) {
        caches.delete(scopedCacheKey);
        return null;
      }
      for (const candidate of cache.candidates) {
        ...
        const chunk = chunks.get(
          documentKey(
            scope.tenantId,
            candidate.documentId,
            `${candidate.documentVersion}\u0000${candidate.chunkId}`,
          ),
        );
        if (
          !projection ||
          projection.state !== "PUBLISHED" ||
          projection.catalogRevision !== candidate.catalogRevision ||
          projection.validFrom !== candidate.projection?.validFrom ||
          projection.validUntil !== candidate.projection?.validUntil ||
          Date.parse(projection.validFrom) > Date.parse(filter.asOf) ||
          Date.parse(filter.asOf) >= Date.parse(projection.validUntil) ||
          !aclAllows(projection.acl, filter.principalRefs) ||
          !sameJson(projection.acl, candidate.projection?.acl) ||
          projection.contentSha256 !==
            candidate.projection?.contentSha256 ||
          !chunk ||
          chunk.active !== true ||
          chunk.state !== "PUBLISHED" ||
          chunk.catalogRevision !== candidate.catalogRevision ||
          chunk.ordinal !== candidate.ordinal ||
          chunk.textSha256 !== candidate.textSha256 ||
          chunk.sourceSha256 !== candidate.sourceSha256 ||
          !sameJson(chunk.location, candidate.location)
        ) {
          caches.delete(scopedCacheKey);
          return null;
        }
    ...
    async writeCache(
      ...
      caches.set(`${scope.tenantId}\u0000${cacheKey}`, {
        ...
        asOf: filter.asOf,
        expiresAt: new Date(
          Date.parse(filter.asOf) + CACHE_TTL_MILLISECONDS,
        ).toISOString(),
...
async function authorize(c06Authorizer, context, resourceId, surface) {
  ...
  validateAuthorizationEvidence(evidence, {
    ...
    workloadActorPrincipalId:
      context.c06ServerContext.workloadActorPrincipalId,
    leafDelegationId: context.authorizationRequest.delegationId,
  });
  return evidence;
}
...
async function resolveCurrentPrincipal(
  ...
  const principalRefs = [...new Set(principal.principalRefs)].sort();
  if (
    !principalRefs.includes(
  ...
  return Object.freeze({
    ...principal,
    principalRefs: Object.freeze(principalRefs),
  });
}

function securityBinding(authorizationEvidence, principal) {
  return Object.freeze({
    tenantId: authorizationEvidence.tenantId,
    surface: authorizationEvidence.surface,
    resourceId: authorizationEvidence.resourceId,
    policyVersion: authorizationEvidence.policyVersion,
    purposeRef: authorizationEvidence.purposeRef,
    humanPrincipalId: authorizationEvidence.humanPrincipalId,
    humanSecurityEpoch: authorizationEvidence.humanSecurityEpoch,
    workloadActorPrincipalId:
      authorizationEvidence.workloadActorPrincipalId,
    workloadActorSecurityEpoch:
      authorizationEvidence.workloadActorSecurityEpoch,
    leafDelegationId: authorizationEvidence.leafDelegationId,
    delegationChainSha256:
      authorizationEvidence.delegationChainSha256,
    principalRefs: principal.principalRefs,
    principalScopeHash: principal.principalScopeHash,
  });
}

async function candidatesAreCurrent(
  catalogReader,
  scope,
  filter,
  candidates,
  ...
  for (const candidate of candidates) {
    ...
    if (document === undefined) {
      ...
      documents.set(key, document ?? null);
    }
    const metadata = document?.metadata;
    const node = document?.nodes?.find(
      ({ nodeId }) => nodeId === candidate.chunkId,
    );
    if (
      !document ||
      document.tenantId !== scope.tenantId ||
      document.documentId !== candidate.documentId ||
      document.documentVersion !== candidate.documentVersion ||
      document.state !== "PUBLISHED" ||
      document.revision !== candidate.catalogRevision ||
      document.parseSha256 !== candidate.projection?.parseSha256 ||
      metadata?.validFrom !== candidate.projection?.validFrom ||
      metadata?.validUntil !== candidate.projection?.validUntil ||
      metadata?.classification !==
        candidate.projection?.classification ||
      Date.parse(metadata?.validFrom) > Date.parse(filter.asOf) ||
      Date.parse(filter.asOf) >= Date.parse(metadata?.validUntil) ||
      !aclAllows(metadata?.acl, filter.principalRefs) ||
      !sameJson(metadata?.acl, candidate.projection?.acl) ||
      document.contentSha256 !== candidate.projection?.contentSha256 ||
      !node ||
      node.nodeType !== "CHUNK" ||
      node.availabilityState !== "PUBLISHED" ||
      node.ordinal !== candidate.ordinal ||
      node.textSha256 !== candidate.textSha256 ||
      node.sourceSha256 !== candidate.sourceSha256 ||
      !sameJson(node.location, candidate.location)
    ) {
      return false;
    }
  }
...
export function createPermissionAwareRag({
  ...
  return Object.freeze({
    ...
    async search(context, request) {
      ...
      const initialSecurityBinding = securityBinding(
        authorizationEvidence,
        principal,
      );
      const principalRefs = principal.principalRefs;
      ...
      const asOf = instant(clock(), "clock");
      const filterBase = {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        candidates = await store.readCache(
          context.tenantScope,
          filter,
          cacheKey,
        );
        ...
        if (
          !Number.isSafeInteger(retrieval?.observedIndexEpoch) ||
          retrieval.observedIndexEpoch < 0 ||
          !Array.isArray(retrieval?.candidates)
        ) {
          fail(
            "INVALID_INDEX",
            "C11 retrieval epoch evidence is invalid.",
          );
        }
      ...
      const finalAuthorizationEvidence = await authorize(
        c06Authorizer,
        context,
        benchmark.searchResourceId,
        SEARCH_SURFACE,
      );
      const finalPrincipal = await resolveCurrentPrincipal(
        principalResolver,
        context,
        finalAuthorizationEvidence,
      );
      if (
        !sameJson(
          initialSecurityBinding,
          securityBinding(finalAuthorizationEvidence, finalPrincipal),
        )
      ) {
        fail(
          "AUTHORIZATION_CHANGED",
          "C11 security binding changed before candidate delivery.",
        );
      }
      ...
      const evidence = qualified.map((candidate) =>
        evidenceRef(candidate, filter, finalAuthorizationEvidence),
      );
      const answerable =
      ...
      await store.recordAudit(context.tenantScope, {
        ...
        authorizationDecisionId:
          finalAuthorizationEvidence.decisionId,
