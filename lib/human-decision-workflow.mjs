...
function sameAuthorityEpochs(left, right) {
  return (
...
export function assertC15EffectCompletion(effect, input) {
  ...
  for (const [name, receipt] of [
    ...
    ["readbackReceipt", input.readbackReceipt],
  ]) {
    if (
      !receipt ||
      ...
      fail("INTEGRITY_VIOLATION", `C15 ${name} is invalid.`);
    }
    assertSyntheticHumanDecisionPayload(receipt);
  }
  const matches =
    input.readbackReceipt.readbackSha256 ===
    effect.expectedReadbackSha256;
  if (
    (input.terminalStatus === "SUCCEEDED" && !matches) ||
    (input.terminalStatus !== "SUCCEEDED" && matches)
  ) {
    fail(
  ...
  if (
    ...
    fail("INTEGRITY_VIOLATION", "C15 compensation receipt is invalid.");
  }
  assertSyntheticHumanDecisionPayload(receipt);
}
...
export function createMemoryHumanDecisionStore({
  ...
  function addAudit(state, audit, now) {
    if (state.auditIntents.has(audit.intentId)) {
    ...
    state.auditIntents.set(intent.intentId, intent);
    state.auditOutbox.set(intent.intentId, {
  ...
  function receipt(state, command, response, now) {
  ...
  async function createArtifact(scope, command) {
    ...
    addAudit(state, command.auditIntent, now);
    receipt(state, command, command.artifact, now);
  ...
  async function createDecision(scope, command) {
    ...
    addAudit(state, command.auditIntent, now);
    receipt(state, command, command.decision, now);
    return viewDecision(command.decision);
  }

  async function getDecision(scope, decisionId) {
    const trusted = validateStoreScope(scope);
    const value = stateFor(trusted.tenantId).decisions.get(decisionId);
    return value ? viewDecision(value) : null;
  }
  ...
  async function withdrawDecision(scope, command) {
    ...
    if (
      [...state.effects.values()].some(
        (effect) => effect.decisionId === current.decisionId,
      )
    ) {
      fail(
        "DECISION_ALREADY_EXECUTING",
        "C15 cannot withdraw a decision after execution was queued.",
      );
    }
    const updated = {
      ...clone(current),
    ...
    state.decisions.set(current.decisionId, updated);
    const now = canonicalInstant(clock(), "clock");
    addAudit(state, command.auditIntent, now);
    receipt(state, command, updated, now);
    return viewDecision(updated);
  }
  ...
  async function queueEffect(scope, command) {
    ...
    addAudit(state, command.auditIntent, now);
    receipt(state, command, command.effect, now);
  ...
  function claim(map, trusted, input, key) {
    validateLease(input);
  ...
  async function claimEffects(scope, input) {
    const trusted = validateStoreScope(scope);
    const state = stateFor(trusted.tenantId);
    return claim(state.effectOutbox, trusted, input, "effectId").map(
      (outbox) => ({
        ...outbox,
  ...
  function activeLease(item, input) {
    return (
      item?.status === "PROCESSING" &&
      item.leasedBy === input.workerId &&
      item.leaseVersion === input.leaseVersion &&
      Date.parse(item.leaseUntil) >= Date.parse(clock())
    );
  }
  ...
  async function completeEffect(scope, input) {
    ...
    addAudit(state, input.auditIntent, effect.updatedAt);
  ...
  async function completeAudit(scope, input) {
    ...
    exactKeys(
      input,
      ["intentId", "workerId", "leaseVersion", "ackIntentId"],
      "audit completion",
    );
