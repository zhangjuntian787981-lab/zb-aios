...
export function createPostgresPermissionAwareRagStore({
  ...
  const sequenceCapabilities = (objectName, privileges) =>
  ...
  const roleCapabilities = {
    aios_c11_projector: [
      ...
      schemaCapability(catalogSchema),
      schemaCapability(schema),
      ...tableCapabilities(
        `${schema}.tenant_index_epoch`,
        ["SELECT", "INSERT", "UPDATE"],
      ),
      ...tableCapabilities(
        `${schema}.document_projection`,
        ["SELECT", "INSERT", "UPDATE"],
      ),
      ...tableCapabilities(
        `${schema}.chunk_index`,
      ...
      ...tableCapabilities(
        `${schema}.retrieval_cache`,
        ["SELECT", "DELETE"],
      ),
      ...tableCapabilities(
        `${schema}.projection_receipt`,
        ["SELECT", "INSERT"],
      ),
      ...tableCapabilities(
        `${catalogSchema}.knowledge_document`,
        ["SELECT"],
      ),
      ...tableCapabilities(
        `${catalogSchema}.source_node`,
        ["SELECT"],
      ),
      functionCapability(
    ...
    aios_c11_query: [
      ...
      ...sequenceCapabilities(
        `${schema}.query_audit_audit_id_seq`,
        ["SELECT", "USAGE"],
      ),
      functionCapability(
        `${scopeSchema}.runtime_scope_allows(text,text)`,
  ...
  async function assertExpectedPrivileges(client, requiredRole) {
    ...
    if (!expected) {
      fail("INVALID_CONFIGURATION", "C11 PostgreSQL role is unknown.");
    }
    ...
    if (result.rows[0]?.privileges_safe !== true) {
      fail(
        ...
        "C11 PostgreSQL pool has unexpected effective privileges.",
      );
    }
  }
