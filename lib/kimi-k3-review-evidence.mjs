...
function isolationShapeValid(isolation, source, tokenEstimateEvidence) {
  const toolKeys = [
    "fileRead",
    "fileWrite",
    "shell",
...
function receiptV6ShapeValid(receipt) {
  return (
    exactKeys(receipt, RECEIPT_KEYS) &&
    receipt.schemaVersion === "independent-model-review-receipt.v6" &&
    receipt.receiptSchemaVersion === "independent-model-review-receipt.v6" &&
    receipt.reviewId === receipt.receiptId &&
    /^imrr_[a-z0-9][a-z0-9_-]{7,127}$/u.test(receipt.receiptId ?? "") &&
    receipt.policyVersion === "2.0.0-candidate.3" &&
    validSha(receipt.policySha256) &&
...
export async function validateKimiK3ReviewReceiptV6({
  receipt,
  transportEvidence,
  tokenEstimateEvidence,
  evidenceResolver,
}) {
  if (!receiptV6ShapeValid(receipt)) {
  ...
  const [
    ...
    await Promise.all([
      ...
      readArtifact(evidenceResolver, { path: RECEIPT_V6_SCHEMA_PATH }),
    ]);
  const estimateValidation = await validateKimiK3TokenEstimateEvidenceV2({
    evidence: tokenEstimateEvidence,
    evidenceResolver,
