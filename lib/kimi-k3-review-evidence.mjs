...
export function createKimiK3ReviewReceiptV8(input) {
  const receipt = {
    ...structuredClone(input),
    receiptSha256: PLACEHOLDER,
  };
...
export function createKimiK3ReviewReceiptV9(input) {
  const receipt = {
    ...structuredClone(input),
    receiptSha256: PLACEHOLDER,
  };
...
function receiptV6BindingsValid(
  bindings,
  semanticValidatorVersion =
  ...
  return (
    ...
    Object.entries(bindings)
      ...
      .every(([, value]) => validSha(value)) &&
    bindings.schemaValidatorVersion === "ajv@8.20.0" &&
    bindings.semanticValidatorVersion === semanticValidatorVersion
  );
}
...
async function validateKimiK3ReviewReceiptCurrent(
  ...
  },
  {
    ...
    runtimeManifestPath,
    runtimeManifestSchemaPath = null,
    runtimeManifestSchemaVersion = null,
    runtimeManifestSchemaLabel = null,
    semanticValidatorVersion =
      "kimi-k3-independent-model-review-semantic-validator.v3",
  },
) {
  ...
  if (!receiptV6BindingsValid(receipt.bindings, semanticValidatorVersion)) {
    ...
    const runtimeManifestSchemaValidation =
      ...
      runtimeManifestSchemaBytes instanceof Uint8Array
        ? await validateIndependentReviewSchemaInstance({
            ...
            label: runtimeManifestSchemaLabel,
          })
        : { ok: false };
    if (
      ...
      runtimeManifest?.schemaVersion !== runtimeManifestSchemaVersion ||
...
export async function validateKimiK3ReviewReceiptV7(input) {
  const validation = await validateKimiK3ReviewReceiptCurrent(input, {
    ...
    runtimeManifestSchemaLabel: "Independent Review Runtime Manifest v3 Schema",
  });
  return {
    ...validation,
...
export async function validateKimiK3ReviewReceiptV8(input) {
  const validation = await validateKimiK3ReviewReceiptCurrent(input, {
    schemaVersion: "independent-model-review-receipt.v8",
    receiptSchemaPath: RECEIPT_V8_SCHEMA_PATH,
    runtimeManifestPath: RUNTIME_MANIFEST_V4_PATH,
    runtimeManifestSchemaPath: RUNTIME_MANIFEST_V4_SCHEMA_PATH,
    runtimeManifestSchemaVersion:
      "independent-review-runtime-dependency-manifest.v4",
    runtimeManifestSchemaLabel: "Independent Review Runtime Manifest v4 Schema",
  });
  return {
    ...validation,
    reasonCodes: validation.reasonCodes.map((reasonCode) =>
      reasonCode.replace("KIMI_K3_RECEIPT_V6_", "KIMI_K3_RECEIPT_V8_"),
    ),
  };
}


export async function validateKimiK3ReviewReceiptV9(input) {
  const validation = await validateKimiK3ReviewReceiptCurrent(input, {
    schemaVersion: "independent-model-review-receipt.v9",
    receiptSchemaPath: RECEIPT_V9_SCHEMA_PATH,
    runtimeManifestPath: RUNTIME_MANIFEST_V5_PATH,
    runtimeManifestSchemaPath: RUNTIME_MANIFEST_V5_SCHEMA_PATH,
    runtimeManifestSchemaVersion:
      "independent-review-runtime-dependency-manifest.v5",
    runtimeManifestSchemaLabel: "Independent Review Runtime Manifest v5 Schema",
    semanticValidatorVersion:
  ...
  return {
    ...validation,
    reasonCodes: validation.reasonCodes.map((reasonCode) =>
      reasonCode.replace("KIMI_K3_RECEIPT_V6_", "KIMI_K3_RECEIPT_V9_"),
    ),
  };
}
