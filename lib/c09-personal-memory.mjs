...
const RETENTION_AUTHORIZATION_FIELDS = Object.freeze([
  "tenantId",
  "operation",
  "actorPrincipalId",
...
export function createMemoryPersonalMemoryStore({ snapshot = null } = {}) {
  ...
  function assertScope(scope, tenantId, principalId) {
    if (
      scope?.trustSource !== "C07_VERIFIED_TENANT_SCOPE" ||
      scope.tenantId !== tenantId ||
      scope.tenantKind !== "SYNTHETIC"
    ...
    if (scope.principalId !== principalId) {
      fail(
        ...
        "Store Principal scope is invalid.",
      );
    }
  }

  function assertRetentionScope(scope, envelope) {
    exactKeys(
      scope,
      [
        "trustSource",
    ...
    if (
      ...
      fail(
        ...
        "C09 retention scope is invalid.",
      );
    }
    validateTenantId(scope.tenantId);
    positiveInteger(scope.lifecycleVersion, "scope.lifecycleVersion");
    identifier(scope.memoryId, MEMORY_ID, "memoryId");
    positiveInteger(scope.expectedVersion, "expectedVersion");
    identifier(
    ...
    nonEmptyString(scope.decisionId, "decisionId", 128);
    reference(scope.evidenceRef, "evidenceRef");
    nonEmptyString(scope.policyVersion, "policyVersion", 128);
    identifier(envelope.eventId, EVENT_ID, "eventId");
    if (!IDEMPOTENCY_KEY.test(envelope.idempotencyKey ?? "")) {
      fail("INVALID_INPUT", "idempotencyKey is invalid.");
    }
    if (!CORRELATION_ID.test(envelope.correlationId ?? "")) {
      fail("INVALID_INPUT", "correlationId is invalid.");
    }
    if (!SHA256.test(envelope.requestHash ?? "")) {
      fail("INVALID_INPUT", "requestHash is invalid.");
    }
    canonicalInstant(envelope.now, "now");
    const authorization = validateRetentionAuthorizationEvidence(
      scope.authorizationEvidence,
      {
        tenantId: scope.tenantId,
        actorPrincipalId: scope.actorPrincipalId,
        actorLifecycleVersion: scope.actorLifecycleVersion,
        actorSecurityEpoch: scope.actorSecurityEpoch,
    ...
    if (
      ...
      envelope.requestHash !== personalMemorySha256({
      ...
      fail(
        "TENANT_SCOPE_VIOLATION",
        "C09 retention evidence is outside its authorized scope.",
      );
    }
  }
  ...
  return Object.freeze({
    async apply(scope, envelope) {
      assertScope(scope, envelope.tenantId, envelope.principalId);
      return locked(async () => {
    ...
    async readCommandReceipt(scope, {
      ...
      assertScope(scope, tenantId, principalId);
    ...
    async materializeExpiry(scope, envelope) {
      assertRetentionScope(scope, envelope);
      return locked(async () => {
        const draft = hydrateState(serializeState(state));
        const memory = draft.memories.get(
          memoryKey(envelope.tenantId, envelope.memoryId),
        );
        if (!memory) {
          fail("MEMORY_NOT_FOUND", "Memory was not found.");
        }
        const result = applyToState(draft, {
          ...clone(envelope),
          principalId: memory.principalId,
          actorPrincipalId: scope.actorPrincipalId,
          authorizationEvidence: clone(scope.authorizationEvidence),
...
function validateRetentionWorkloadIdentity(identity, tenantId) {
  exactKeys(
    identity,
    [
      "trustSource",
    ...
    "retention workload identity",
  );
  if (
    identity.trustSource !== "C05_VERIFIED_WORKLOAD_IDENTITY" ||
    identity.tenantId !== tenantId ||
    identity.tenantKind !== "SYNTHETIC" ||
    identity.principalKind !== "SERVICE"
  ) {
    fail(
      "IDENTITY_BINDING_INVALID",
      "C09 retention workload identity is invalid.",
    );
  }
  identifier(identity.principalId, PRINCIPAL_ID, "actorPrincipalId");
  positiveInteger(identity.lifecycleVersion, "actorLifecycleVersion");
  positiveInteger(identity.securityEpoch, "actorSecurityEpoch");
  return clone(identity);
}

function validateRetentionAuthorization(
  decision,
  { tenantId, identity, memoryId, expectedVersion },
) {
  exactKeys(
    decision,
    [
  ...
  if (
    decision.allowed !== true ||
    decision.tenantId !== tenantId ||
    decision.operation !== "C09_RETENTION_MATERIALIZE_EXPIRY" ||
    decision.actorPrincipalId !== identity.principalId ||
    decision.resourceId !== memoryId ||
    decision.expectedVersion !== expectedVersion
  ) {
    fail("FORBIDDEN", "C06 denied C09 retention materialization.");
  }
  nonEmptyString(decision.decisionId, "decisionId", 128);
  reference(decision.evidenceRef, "evidenceRef");
  nonEmptyString(decision.policyVersion, "policyVersion", 128);
  return {
    ...
    actorLifecycleVersion: identity.lifecycleVersion,
    actorSecurityEpoch: identity.securityEpoch,
    resourceId: memoryId,
    expectedVersion,
    decisionId: decision.decisionId,
    evidenceRef: decision.evidenceRef,
...
function validateRetentionAuthorizationEvidence(
  evidence,
  {
    tenantId,
    actorPrincipalId,
  ...
  exactKeys(
    evidence,
    RETENTION_AUTHORIZATION_FIELDS,
    "retention authorization evidence",
  );
  if (
    evidence.tenantId !== tenantId ||
    evidence.operation !== "C09_RETENTION_MATERIALIZE_EXPIRY" ||
    evidence.actorPrincipalId !== actorPrincipalId ||
    evidence.actorLifecycleVersion !== actorLifecycleVersion ||
    evidence.actorSecurityEpoch !== actorSecurityEpoch ||
    evidence.resourceId !== memoryId ||
    evidence.expectedVersion !== expectedVersion
  ) {
    fail(
      "TENANT_SCOPE_VIOLATION",
      "C09 retention authorization evidence is invalid.",
    );
  }
  validateTenantId(evidence.tenantId);
  identifier(
  ...
  positiveInteger(
    evidence.actorLifecycleVersion,
    "authorization.actorLifecycleVersion",
  );
  positiveInteger(
    evidence.actorSecurityEpoch,
    "authorization.actorSecurityEpoch",
  );
  identifier(evidence.resourceId, MEMORY_ID, "authorization.resourceId");
  positiveInteger(
    evidence.expectedVersion,
    "authorization.expectedVersion",
  );
  nonEmptyString(evidence.decisionId, "authorization.decisionId", 128);
  reference(evidence.evidenceRef, "authorization.evidenceRef");
  nonEmptyString(
...
export function createPersonalMemoryRetentionWorker({
  tenantRegistry,
  ...
  if (
    ...
    typeof workloadIdentityProvider?.resolve !== "function" ||
    typeof authorizer?.enforce !== "function" ||
    typeof tenantScopeFactory !== "function" ||
    typeof store?.materializeExpiry !== "function" ||
    typeof clock !== "function" ||
    typeof idFactory !== "function"
  ) {
    fail(
      "INVALID_CONFIGURATION",
      "C09 retention worker dependencies are incomplete.",
    );
  }

  return Object.freeze({
    async materializeExpiry(command) {
      exactKeys(
        command,
        [
          "tenantId",
      ...
      if (!CORRELATION_ID.test(command.correlationId ?? "")) {
        fail("INVALID_INPUT", "correlationId is invalid.");
      }
      const identity = validateRetentionWorkloadIdentity(
        await workloadIdentityProvider.resolve({
          tenantId: command.tenantId,
          operation: "C09_RETENTION_MATERIALIZE_EXPIRY",
        }),
        command.tenantId,
      );
      const authorization = validateRetentionAuthorization(
      ...
      const finalIdentity = validateRetentionWorkloadIdentity(
        await workloadIdentityProvider.resolve({
          tenantId: command.tenantId,
          operation: "C09_RETENTION_MATERIALIZE_EXPIRY",
        }),
      ...
      if (
        finalIdentity.principalId !== identity.principalId ||
        finalIdentity.lifecycleVersion !== identity.lifecycleVersion ||
        finalIdentity.securityEpoch !== identity.securityEpoch
      ) {
        fail(
          "IDENTITY_CHANGED",
          "C09 retention workload identity changed during authorization.",
        );
      }
      ...
      if (
        ...
        fail(
          "TENANT_SCOPE_VIOLATION",
          "C03 retention Tenant admission is invalid.",
        );
      }
      ...
      const requestHash = personalMemorySha256({
        ...
        memoryId: command.memoryId,
        expectedVersion: command.expectedVersion,
        actorPrincipalId: finalIdentity.principalId,
        actorLifecycleVersion: finalIdentity.lifecycleVersion,
        actorSecurityEpoch: finalIdentity.securityEpoch,
        idempotencyKey: command.idempotencyKey,
        correlationId: command.correlationId,
      });
      return store.materializeExpiry(
        {
          ...tenantScope,
...
export function createPersonalMemoryService({
  ...
  async function resolveIdentity(context, request) {
  ...
  async function resolve(context, request, operation, correlationId, resource) {
    ...
    const finalIdentity = await resolveIdentity(context, request);
    ...
    const scope = {
      ...tenantScope,
      principalId: finalIdentity.humanSubject.principalId,
  ...
  return Object.freeze({
    ...
    async recall(context, request) {
      ...
      for (const item of metadata) {
        ...
        let finalItemIdentity;
        try {
          finalItemIdentity = await resolveIdentity(context, request);
        } catch (error) {
          if (
            error instanceof PersonalMemoryError &&
        ...
        if (!sameActionIdentity(trusted.identity, finalItemIdentity)) {
          continue;
        }
        ...
        if (memory) {
          if (
            ...
            memory.principalId !== trusted.principalId ||
            memory.memoryId !== item.memoryId ||
            memory.category !== item.category ||
            memory.state !== "CONFIRMED" ||
            memory.version !== item.version ||
            memory.contentSha256 !== item.contentSha256 ||
            typeof memory.content !== "string" ||
            personalMemorySha256(memory.content) !== memory.contentSha256
          ) {
            fail(
              "INTEGRITY_VIOLATION",
              "Recall content does not match its authorized metadata.",
            );
          }
          let finalItemDecision;
          try {
            finalItemDecision = validateAuthorization(
              await authorizer.enforce({
                operation: "C09_RECALL_ITEM",
                tenantId: context.tenantId,
                identity: clone(trusted.identity),
          ...
          try {
            deliveryIdentity = await resolveIdentity(context, request);
          } catch (error) {
            if (
              error instanceof PersonalMemoryError &&
              error.code === "IDENTITY_BINDING_INVALID"
          ...
          if (!sameActionIdentity(trusted.identity, deliveryIdentity)) {
            continue;
          }
          memories.push({
            memoryId: memory.memoryId,
            ...
            version: memory.version,
            expiresAt: memory.expiresAt,
            authorizationEvidence: finalItemDecision,
          });
        }
      }
      return { principalId: trusted.principalId, memories };
