...
function applyToState(state, envelope) {
  ...
  } else if (envelope.operation === "EXPIRE_MEMORY") {
    const memory = requireOwnedMemory(state, envelope);
    if (
...
function serializeState(state) {
  ...
  return {
    ...
    memories,
    checkpoints: [...state.checkpoints.values()].map(clone),
...
function validateCommand(command, catalog, tenantId, idFactory) {
  ...
  if (
    ["CONFIRM_CANDIDATE", "EXPIRE_MEMORY", "DELETE_MEMORY"].includes(
      command.kind,
    )
  ) {
    ...
    if (
      command.kind === "CONFIRM_CANDIDATE" &&
      command.explicitConfirmation !== true
    ) {
      fail(
        "EXPLICIT_CONFIRMATION_REQUIRED",
        "Only explicit owner confirmation can activate a Candidate.",
      );
    }
    return {
      ...base,
  ...
  if (command.kind === "CORRECT_MEMORY") {
    identifier(command.memoryId, MEMORY_ID, "memoryId");
    positiveInteger(command.expectedVersion, "expectedVersion");
    if (command.explicitConfirmation !== true) {
      fail(
        "EXPLICIT_CONFIRMATION_REQUIRED",
        "A correction requires explicit owner confirmation.",
      );
    }
    return {
      ...base,
