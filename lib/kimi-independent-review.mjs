...
import {
  ...
  validateKimiK3HistoricalEvidenceReferences,
} from "./kimi-k3-independent-review.mjs";
import {
...
const MATERIAL_V4_BINDING_KEYS = [
  "reviewBundle",
  "reviewerPrompt",
...
function validHistoricalEvidenceIndexBinding(binding) {
  return (
    exactKeys(binding, ["path", "byteLength", "sha256", "indexDigest"]) &&
    binding.path === kimiK3ReviewMaterialPathsV3.historicalEvidenceIndex &&
    Number.isInteger(binding.byteLength) &&
    binding.byteLength > 0 &&
    binding.byteLength <= 1024 * 1024 &&
    validSha(binding.sha256) &&
    validSha(binding.indexDigest)
  );
}

function sortedPathCoverage(values) {
...
function materialV2ManifestReasonCodes(material, sectionBytes = null) {
  const reasonCodes = [];
  const isV4 = material?.schemaVersion === "independent-review-material.v4";
  const isV3 = material?.schemaVersion === "independent-review-material.v3";
  const expectedContextBudget = isV4
    ? 4 * 1024 * 1024
    : isV3
      ? 1024 * 1024
      : 576 * 1024;
  const maximumSectionByteLength = isV4
    ? 4 * 1024 * 1024
    : 1024 * 1024;
  const expectedMaterialKeys = isV4 ? MATERIAL_V4_KEYS : MATERIAL_KEYS;
  const expectedBindingKeys = isV4
    ? MATERIAL_V4_BINDING_KEYS
    : [
        "reviewBundle",
        "reviewerPrompt",
        "canonicalOutputSchema",
  ...
  if (
    ...
    !exactKeys(material.bindings, expectedBindingKeys) ||
    ...
    material.totalSectionUtf8ByteLength > maximumSectionByteLength ||
    material.contextBudgetUtf8Bytes !== expectedContextBudget ||
    (isV4 &&
  ...
  if (
    isV4 &&
    (!validHistoricalEvidenceIndexBinding(
      material?.bindings?.historicalEvidenceIndex,
    ) ||
      !validByteBinding(
        material?.bindings?.historicalEvidenceIndexSchema,
      ) ||
      material.bindings.historicalEvidenceIndexSchema.path !==
        kimiK3ReviewMaterialPathsV3.historicalEvidenceIndexSchema ||
      !validateKimiK3HistoricalEvidenceReferences(
        material.priorReviewEvidenceReferences,
      ).ok ||
      sha256Value(material.priorReviewEvidenceReferences) !==
        material.priorReviewEvidenceReferenceSetSha256 ||
      !validSha(material.reviewedPathCoverageSha256))
  ) {
    reasonCodes.push(
  ...
  for (const descriptor of material?.sections ?? []) {
    if (
      ...
      descriptor.byteLength > maximumSectionByteLength ||
...
function k3MaterialContractForReceipt(material) {
  const receiptPath = material?.bindings?.canonicalReceiptSchema?.path;
  if (receiptPath === kimiK3ReviewMaterialPathsV7.receiptSchema) {
    return {
      governancePaths: kimiK3ReviewMaterialGovernancePathsV7,
      materialPaths: kimiK3ReviewMaterialPathsV7,
    };
  }
  if (receiptPath === kimiK3ReviewMaterialPathsV6.receiptSchema) {
    return {
      governancePaths: kimiK3ReviewMaterialGovernancePathsV6,
      materialPaths: kimiK3ReviewMaterialPathsV6,
    };
  }
  if (receiptPath === kimiK3ReviewMaterialPathsV5.receiptSchema) {
    return {
      governancePaths: kimiK3ReviewMaterialGovernancePathsV5,
      materialPaths: kimiK3ReviewMaterialPathsV5,
    };
  }
  if (receiptPath === kimiK3ReviewMaterialPathsV4.receiptSchema) {
    return {
      governancePaths: kimiK3ReviewMaterialGovernancePathsV4,
      materialPaths: kimiK3ReviewMaterialPathsV4,
    };
  }
  return {
    governancePaths: kimiK3ReviewMaterialGovernancePathsV3,
    materialPaths: kimiK3ReviewMaterialPathsV3,
  };
}
...
function expectedSectionCoverage(expected, material) {
  ...
  const isV4 = material?.schemaVersion === "independent-review-material.v4";
  const isV3 = material?.schemaVersion === "independent-review-material.v3";
  const k3Contract = isV4 ? k3MaterialContractForReceipt(material) : null;
  const governancePaths = isV4
    ? k3Contract.governancePaths
    : isV3
      ? kimiK3ReviewMaterialGovernancePathsV2
      : kimiIndependentReviewMaterialGovernancePaths;
  const materialPaths = isV4
    ? k3Contract.materialPaths
    : isV3
      ? kimiK3ReviewMaterialPathsV2
      : MATERIAL_PATHS;
  if (
    ...
    governanceBindings.length !== governancePaths.length ||
  ...
  for (const binding of governanceBindings) {
    if (
      !exactKeys(binding, BYTE_BINDING_KEYS) ||
      !governancePaths.includes(binding.path) ||
      !Number.isInteger(binding.byteLength) ||
      binding.byteLength <= 0 ||
      !validSha(binding.sha256) ||
  ...
  if (
    !add(
      "REVIEW_BUNDLE",
      materialPaths.reviewBundle,
      expected.reviewBundleBytesSha256,
      expected.reviewBundleByteLength,
    ) ||
    !add("PATCH", materialPaths.patch, bundle.source?.diffSha256)
  ) {
    return null;
  }
  ...
  if (
    !add(
      "GOVERNANCE",
      materialPaths.modelVisibleProtocol,
...
async function validateIndependentReviewMaterialV2({
  ...
  const isV3 = material?.schemaVersion === "independent-review-material.v3";
  const materialPaths = isV4
    ? k3MaterialContractForReceipt(material).materialPaths
    : isV3
      ? kimiK3ReviewMaterialPathsV2
      : MATERIAL_PATHS;
  ...
  const expectedBindings =
    ...
    (expected?.bundle
      ? {
          reviewBundle: {
            path: materialPaths.reviewBundle,
          ...
          ...(isV4
            ? {
                historicalEvidenceIndex: {
                  path: kimiK3ReviewMaterialPathsV3.historicalEvidenceIndex,
                  byteLength: expected.historicalEvidenceIndexByteLength,
                  sha256: expected.historicalEvidenceIndexBytesSha256,
                  indexDigest: expected.historicalEvidenceIndexDigest,
                },
                historicalEvidenceIndexSchema: {
                  path:
                    kimiK3ReviewMaterialPathsV3.historicalEvidenceIndexSchema,
                  byteLength:
                    expected.historicalEvidenceIndexSchemaByteLength,
                  sha256:
                    expected.historicalEvidenceIndexSchemaBytesSha256,
                },
              }
            : {}),
        }
  ...
  if (isV4) {
    const references = material?.priorReviewEvidenceReferences;
    const sourceSubjects = new Map(
      (expected?.bundle?.sourceSubjects ?? []).map((subject) => [
        subject.path,
        subject,
      ]),
    );
    const rawReviewedPaths = (material?.sections ?? [])
      .filter(
        ({ kind, path }) =>
          ...
          expected?.bundle?.reviewedPaths?.includes(path),
      )
      .map(({ kind, path }) => ({
        path,
        coverageKind: kind,
    ...
    const referencedReviewedPaths = (references ?? []).map(({ path }) => ({
      path,
      coverageKind: "PRIOR_REVIEW_EVIDENCE_REFERENCE",
      sourceBlobSha256: sourceSubjects.get(path)?.blobSha256,
    }));
    const coverage = sortedPathCoverage([
      ...rawReviewedPaths,
      ...referencedReviewedPaths,
    ]);
    const coveragePaths = coverage.map(({ path }) => path);
    if (
      !Array.isArray(expected?.bundle?.reviewedPaths) ||
      !Array.isArray(expected?.priorReviewEvidenceReferences) ||
      !coverage.every(({ sourceBlobSha256 }) =>
        validSha(sourceBlobSha256),
      ) ||
      !sameCanonical(references, expected.priorReviewEvidenceReferences) ||
      !unique(coveragePaths) ||
      canonicalize(coveragePaths) !==
        canonicalize(expected.bundle.reviewedPaths) ||
      sha256Value(coverage) !== material.reviewedPathCoverageSha256
    ) {
      reasonCodes.push(
  ...
  if (
    ...
    reasonCodes.push(
      isV4
        ? "KIMI_REVIEW_MATERIAL_RESOURCE_CEILING_MISMATCH"
        : "KIMI_REVIEW_MATERIAL_CONTEXT_BUDGET_MISMATCH",
    );
  }
  if (
    isV4 &&
    expected?.resourceCeilingBasis !== undefined &&
    material?.resourceCeilingBasis !== expected?.resourceCeilingBasis
  ) {
    reasonCodes.push("KIMI_REVIEW_MATERIAL_RESOURCE_CEILING_MISMATCH");
  }
  return {
    ok: reasonCodes.length === 0,
    reasonCodes: [...new Set(reasonCodes)].sort(),
