...
function materialV2ManifestReasonCodes(material, sectionBytes = null) {
  const reasonCodes = [];
  const isK3 = material?.schemaVersion === "independent-review-material.v3";
  const expectedContextBudget = isK3 ? 1024 * 1024 : 576 * 1024;
  if (
...
function expectedSectionCoverage(expected, material) {
  ...
  const materialPaths = isK3 ? kimiK3ReviewMaterialPaths : MATERIAL_PATHS;
  if (
    !bundle ||
...
async function validateIndependentReviewMaterialV2({
  ...
  const materialPaths =
    material?.schemaVersion === "independent-review-material.v3"
      ? kimiK3ReviewMaterialPaths
      : MATERIAL_PATHS;
