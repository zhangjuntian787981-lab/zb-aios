...
function exactRequest(request) {
  ...
  if (
    ...
    fail("INVALID_INPUT", "C08 Outbox Worker request is invalid.");
  }
}

export function createC08OutboxWorker({ store, publisher }) {
  if (
    ...
    fail(
      ...
      "C08 Outbox Worker dependencies are invalid.",
    );
  }
  ...
  async function runOnce(scope, request) {
    ...
    for (const claim of claims) {
      let acknowledgement;
      try {
        acknowledgement = await publisher.publish({
        ...
        if (acknowledgement?.eventId !== claim.eventId) {
          fail(
            "PUBLISH_ACK_INVALID",
            "Publisher acknowledgement does not match the Event.",
          );
        }
      } catch {
