export class C14C06AuthorizationError extends Error {
  constructor(code, message) {
    super(message);
    this.name = "C14C06AuthorizationError";
    this.code = code;
  }
}

function fail(code, message) {
  throw new C14C06AuthorizationError(code, message);
}
...
export function createC14C06Authorizer({ authorizationFacade }) {
  ...
  return Object.freeze({
    async enforce(serverContext, request, descriptor) {
      ...
      if (
        ...
        fail("ACCESS_DENIED", "C06 denied the C14 operation.");
      }
      for (const field of [
        "decisionId",
        "evidenceRef",
