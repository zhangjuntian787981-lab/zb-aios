...
const CLOSED_OPERATION_DEFINITIONS = Object.freeze({
  "synthetic.approval.status.get": {
...
function normalizeOperation(value) {
  ...
  if (
    !CLOSED_OPERATION_DEFINITIONS[value.operationId] ||
    toolGatewaySha256(value) !==
      toolGatewaySha256(
        CLOSED_OPERATION_DEFINITIONS[value.operationId],
      )
  ) {
    fail("INVALID_CATALOG", "C16 operation definition is not frozen.");
  }
...
export function createSyntheticToolCatalog(document) {
  ...
  if (
    ...
    document.catalogVersion !== "c16-synthetic-tools-v1" ||
...
export function validateC16SyntheticCompletion({
  call,
  result,
  receipt,
}) {
  const exactAdapterKeys = (value, allowed, field) => {
    if (
      !value ||
      typeof value !== "object" ||
      Array.isArray(value) ||
      ...
      fail(
        ...
        `C16 ${field} schema is invalid.`,
      );
    }
  };
  const operation = CLOSED_OPERATION_DEFINITIONS[call?.operationId];
  if (
    !operation ||
    call?.tenantKind !== "SYNTHETIC" ||
    !SYNTHETIC_TENANT_ID.test(call?.tenantId ?? "") ||
    !/^tcl_[0-9a-f-]{36}$/.test(call?.callId ?? "") ||
    !/^sha256:[a-f0-9]{64}$/.test(call?.effectKey ?? "") ||
    call?.adapterVersion !== operation.adapterVersion
  ) {
    fail("ADAPTER_RESULT_INVALID", "C16 ToolCall binding is invalid.");
  }
  plainObject(call.normalizedParams, "call.normalizedParams");
  if (call.operationId === "synthetic.approval.status.get") {
    exactAdapterKeys(result, ["approvalRef", "status"], "result");
    if (
      result.approvalRef !== call.normalizedParams.approvalRef ||
      ![
      ...
      fail("ADAPTER_RESULT_INVALID", "C16 approval result is invalid.");
    }
  } else if (call.operationId === "synthetic.erp.order.get") {
    exactAdapterKeys(result, ["orderRef", "state"], "result");
    if (
      result.orderRef !== call.normalizedParams.orderRef ||
      ![
        "SYNTHETIC_OPEN",
        "SYNTHETIC_RELEASED",
        "SYNTHETIC_CLOSED",
      ...
      fail("ADAPTER_RESULT_INVALID", "C16 ERP result is invalid.");
    }
  } else {
    exactAdapterKeys(
      result,
      ["metricCode", "period", "value", "unit"],
      "result",
    );
    if (
      result.metricCode !== call.normalizedParams.metricCode ||
      result.period !== call.normalizedParams.period ||
      typeof result.value !== "number" ||
      !Number.isFinite(result.value) ||
      result.unit !==
        (result.metricCode === "on_time_delivery_rate"
          ? "PERCENT"
          : "COUNT")
    ) {
      fail("ADAPTER_RESULT_INVALID", "C16 BI result is invalid.");
    }
  }
  exactAdapterKeys(
    receipt,
  ...
  if (
    receipt.schemaVersion !== "c16-c0-tool-receipt.v1" ||
    receipt.tenantId !== call.tenantId ||
    receipt.tenantKind !== "SYNTHETIC" ||
    receipt.callId !== call.callId ||
    receipt.operationId !== call.operationId ||
    receipt.effectKey !== call.effectKey ||
    receipt.adapterVersion !== call.adapterVersion ||
    receipt.resultSha256 !== toolGatewaySha256(result) ||
    receipt.networkRequestCount !== 0 ||
    receipt.externalEffectCount !== 0 ||
    typeof receipt.replayed !== "boolean" ||
    !/^sha256:[a-f0-9]{64}$/.test(receipt.receiptSha256 ?? "") ||
    receipt.receiptSha256 !== toolGatewaySha256(receiptBase)
  ) {
    fail("ADAPTER_RESULT_INVALID", "C16 C0 receipt is invalid.");
  }
  return deepFreeze({
    result: clone(result),
...
function authorizationAuthoritySha256(value) {
  return toolGatewaySha256({
...
export function createMemoryToolGatewayStore({
  ...
  return Object.freeze({
    ...
    async beginExecution(scope, command) {
      ...
      if (
        command.tenantId !== scope.tenantId ||
        command.call?.tenantLifecycleVersion !== scope.lifecycleVersion
      ) {
        fail(
          "TENANT_SCOPE_VIOLATION",
...
export function createToolGateway({
  ...
  return Object.freeze({
    ...
    async confirm(serverContext, request) {
      ...
      const stableAuthorizationSha256 =
        authorizationAuthoritySha256(access.authorization);
      ...
      const confirmationBase = {
        ...
        tenantId: serverContext.tenantId,
        tenantKind: "SYNTHETIC",
        tenantLifecycleVersion: access.tenant.lifecycleVersion,
    ...
    async execute(serverContext, request) {
      ...
      if (
        ...
        confirmation.tenantLifecycleVersion !==
          access.tenant.lifecycleVersion ||
      ...
      const currentAuthorizationAuthoritySha256 =
        authorizationAuthoritySha256(access.authorization);
      if (
      ...
      if (
        confirmation.authorizationAuthoritySha256 !==
          authorizationAuthoritySha256(
            confirmation.authorizationBinding,
          ) ||
        confirmation.authorizationAuthoritySha256 !==
          currentAuthorizationAuthoritySha256
      ) {
        fail(
          "CONFIRMATION_AUTHORITY_CHANGED",
          "C16 confirmation authorization changed.",
        );
      }
      ...
      const requestHash = toolGatewaySha256({
        ...
        tenantLifecycleVersion: call.tenantLifecycleVersion,
        authorizationAuthoritySha256:
      ...
      try {
        ...
        exactKeys(outcome, ["result", "receipt"], "adapter outcome");
        outcome = validateC16SyntheticCompletion({
          call: adapterCall,
