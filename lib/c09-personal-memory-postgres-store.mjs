...
import { PersonalMemoryError } from "./c09-personal-memory.mjs";
...
function translate(error) {
  ...
  if (error?.code === "23505") {
    return mapped(
      ...
      "Unique C09 state already exists.",
    );
  }
...
export function createPostgresPersonalMemoryStore({
  ...
  if (
    ...
    new Set([runtimePool, tenantScopePool, principalScopePool]).size !== 3 ||
  ...
  async function runScoped(scope, principalId, begin, reducer) {
    if (
    ...
    try {
      ...
      const [tenant, principal] = await Promise.all([
        ...
        signPrincipalScope(scope, principalId, backendPid, transactionId),
      ]);
      await client.query(
  ...
  async function withSerializable(scope, principalId, reducer) {
  ...
  async function insertEvent(client, envelope, event) {
    await client.query(
      `INSERT INTO ${table("memory_event")} (
         ...
         authorization_evidence,correlation_id,created_at
       ) VALUES (
  ...
  async function applyAction(client, envelope, profile) {
    ...
    if (
      ["EXPIRE_MEMORY", "DELETE_MEMORY"].includes(envelope.operation)
    ) {
      const memory = await findMemory(client, envelope);
      const expiration = envelope.operation === "EXPIRE_MEMORY";
      if (
        expiration &&
        (
  ...
  return Object.freeze({
    async apply(scope, envelope) {
      ...
      return withSerializable(scope, envelope.principalId, async (client) => {
        await client.query(
        ...
        if (replay.rowCount === 1) {
          if (replay.rows[0].request_hash !== envelope.requestHash) {
            fail("IDEMPOTENCY_CONFLICT", "Idempotency key was reused.");
          }
          return json(replay.rows[0].result);
        }
        const profile = await ensureProfile(client, envelope);
        const result = await applyAction(client, envelope, profile);
        await client.query(
    ...
    async listRecallMetadata(scope, {
      ...
      return runScoped(
        ...
        async (client) => {
          const result = await client.query(
            `SELECT memory_id,category,content_sha256,version,expires_at
               FROM ${table("personal_memory")}
              WHERE tenant_id=$1 AND principal_id=$2
                AND state='CONFIRMED' AND expires_at>$3
      ...
      return runScoped(
        ...
        async (client) => {
          const result = await client.query(
            `SELECT * FROM ${table("personal_memory")}
              WHERE tenant_id=$1 AND principal_id=$2 AND memory_id=$3
                AND version=$4 AND state='CONFIRMED' AND expires_at>$5`,
    ...
    async readCheckpoint(scope, {
      ...
      return runScoped(
        ...
        async (client) => {
          const result = await client.query(
            `SELECT * FROM ${table("conversation_checkpoint")}
              WHERE tenant_id=$1 AND principal_id=$2 AND checkpoint_id=$3`,
            [tenantId, principalId, checkpointId],
          );
