...
export function createPostgresPersonalMemoryStore({
  ...
  const roleCapabilities = {
    aios_c09_runtime: [
      ...
      schemaCapability(tenantScopeSchema),
      ...tableCapabilities(
        `${schema}.personal_profile`,
        ["SELECT", "INSERT", "UPDATE"],
      ),
      ...tableCapabilities(
        `${schema}.personal_memory`,
        ["SELECT", "INSERT", "UPDATE"],
      ),
      ...tableCapabilities(
        `${schema}.conversation_checkpoint`,
        ["SELECT", "INSERT", "UPDATE"],
      ),
      ...tableCapabilities(
        `${schema}.memory_event`,
      ...
      functionCapability(
        `${tenantScopeSchema}.runtime_scope_allows(text,text)`,
      ),
      functionCapability(`${tenantScopeSchema}.acquire_runtime_fence()`),
      functionCapability(
        `${schema}.runtime_principal_allows(text,text,text)`,
      ),
    ],
    aios_c07_scope_runtime: [
      schemaCapability(tenantScopeSchema),
      functionCapability(
        `${tenantScopeSchema}.issue_runtime_scope_signature(text,text,bigint,text,text,text,text,integer,xid8,integer,uuid)`,
      ),
    ],
    aios_c09_scope_runtime: [
      schemaCapability(schema),
      functionCapability(
        `${schema}.issue_principal_scope_signature(text,text,text,bigint,bigint,integer,xid8,integer,uuid)`,
      ),
    ],
    aios_c09_retention_runtime: [
      ...
      functionCapability(`${tenantScopeSchema}.acquire_runtime_fence()`),
      functionCapability(
        `${schema}.materialize_due_expiry(text,text,bigint,text,text,text,text,text,bigint,bigint,jsonb)`,
      ),
    ],
  };
  ...
  async function assertExpectedPrivileges(client, requiredRole) {
    ...
    if (!expected) {
      fail("INVALID_CONFIGURATION", "C09 PostgreSQL role is unknown.");
    }
    ...
    if (result.rows[0]?.privileges_safe !== true) {
      fail(
        ...
        "C09 PostgreSQL pool has unexpected effective privileges.",
      );
    }
  }
  ...
  async function runRetentionScoped(scope, reducer) {
    ...
    if (
      ...
      Object.keys(authorization).sort().join(",") !==
        RETENTION_AUTHORIZATION_FIELDS.join(",") ||
      ...
      authorization?.actorSecurityEpoch !== scope.actorSecurityEpoch ||
