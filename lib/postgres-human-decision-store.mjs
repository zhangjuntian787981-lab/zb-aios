...
function databaseFailure(error) {
  ...
  if (error?.constraint === "c15_effect_decision_active_guard") {
    return new PostgresHumanDecisionStoreError(
      "DECISION_NOT_ACTIVE",
      "C15 decision cannot authorize effect execution.",
    );
  }
  if (error?.constraint === "c15_decision_execution_started_guard") {
    return new PostgresHumanDecisionStoreError(
      "DECISION_ALREADY_EXECUTING",
      "C15 cannot withdraw after effect execution started.",
    );
  }
  ...
  if (error?.constraint === "c15_claimed_effect_guard") {
    return new PostgresHumanDecisionStoreError(
      "INTEGRITY_VIOLATION",
      "C15 claimed Effect binding changed.",
      { cause: error },
    );
  }
...
export function createPostgresHumanDecisionStore({
  ...
  const effectWorkerFunctions = [
    ...
    functionCapability(
      `${schema}.complete_effect(text,text,text,text,bigint,text,text,jsonb,jsonb,jsonb,text)`,
    ),
    functionCapability(
      `${schema}.assert_effect_execution_authorized(text,text,text,text,bigint,text)`,
    ),
    functionCapability(
  ...
  function validateExecutionAuthorization(input) {
    ...
    if (
      ...
      !input.effectId ||
      input.effectId.length > 128 ||
      !SHA256.test(input.effectSha256 ?? "") ||
      typeof input.workerId !== "string" ||
      !input.workerId.trim() ||
      input.workerId.length > 128 ||
      ...
      fail("INVALID_INPUT", "C15 effect execution authorization is invalid.");
    }
  }

  async function assertEffectExecutable(scope, input) {
    const trusted = validateScope(scope);
    validateExecutionAuthorization(input);
    try {
      return await runScoped(
        effectWorkerPool,
        "aios_c15_effect_worker",
        ...
        async (client) => {
          await client.query(
            `SELECT "${schema}"."assert_effect_execution_authorized"(
               $1,$2,$3,$4,$5,$6
             )`,
            [
              trusted.tenantId,
  ...
  function validateCompletion(trusted, input) {
    const claimedEffect = input?.effect
      ? without(input.effect, "replayed")
      : null;
    if (
      !claimedEffect ||
      claimedEffect.tenantId !== trusted.tenantId ||
      claimedEffect.effectId !== input?.effectId ||
      !SHA256.test(claimedEffect.effectSha256 ?? "") ||
      humanDecisionSha256(
        without(claimedEffect, "effectSha256"),
      ) !== claimedEffect.effectSha256 ||
      typeof input?.effectId !== "string" ||
      !input.effectId ||
      typeof input?.workerId !== "string" ||
      input.workerId.trim().length < 1 ||
  ...
  async function completeAudit(scope, input) {
    ...
    if (
      ...
      fail("INVALID_INPUT", "C15 audit completion is invalid.");
    }
    if (
