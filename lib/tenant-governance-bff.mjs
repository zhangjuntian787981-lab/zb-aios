...
export function c02GovernanceCommandSha256(scope, command) {
  return c02Sha256({
    schemaVersion: "c02-governance-command-evidence.v1",
    tenantId: scope.tenantId,
    correlationId: scope.correlationId,
    humanPrincipalId: scope.humanPrincipalId,
    workloadActorPrincipalId: scope.workloadActorPrincipalId,
    authorizationDecisionId: scope.decisionId,
    authorizationEvidenceRef: scope.evidenceRef,
    authorizationPolicyVersion: scope.policyVersion,
    command,
  });
}

export function c02GovernanceResultSha256(receipt) {
  return c02Sha256({
    schemaVersion: "c02-governance-result-evidence.v1",
...
async function verifyAuditReadback(
  auditVerifier,
  scope,
  request,
  command,
  ...
  const expected = {
    ...
    correlationId: request.correlationId,
    commandSha256: c02GovernanceCommandSha256(scope, command),
    resultSha256: c02GovernanceResultSha256(receipt),
  };
  let proof;
  try {
    proof = await auditVerifier.verify(scope, deepFreeze(clone(expected)));
  } catch {
    fail(
      "AUDIT_EVIDENCE_INVALID",
      "C02 could not read back the C18 audit event.",
    );
  }
  exactKeys(
    proof,
  ...
  if (
    proof.trustSource !== "C18_IMMUTABLE_EVENT_READBACK" ||
    proof.tenantId !== scope.tenantId ||
    proof.auditRef !== expected.auditRef ||
    proof.commandSha256 !== expected.commandSha256 ||
    proof.resultSha256 !== expected.resultSha256 ||
    !Number.isSafeInteger(proof.sequence) ||
    proof.sequence < 1 ||
    !SHA256.test(proof.eventHash ?? "") ||
    !SHA256.test(proof.payloadSha256 ?? "")
  ) {
    fail(
      "AUDIT_EVIDENCE_INVALID",
      "C02 C18 audit evidence is unbound.",
    );
  }
  nonEmpty(proof.eventId, "auditVerification.eventId", 256);
...
export function createTenantGovernanceBff({
  ...
  if (
    ...
    typeof auditVerifier?.verify !== "function"
  ) {
  ...
  return Object.freeze({
    ...
    async mutate(serverContext, originalRequest) {
      ...
      let receipt;
      try {
        receipt = await corePort.mutate(scope, command);
      } catch {
