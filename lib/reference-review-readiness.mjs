...
import applicabilityEvidenceV2Schema from "../implementation/governance/schemas/reference-applicability-evidence.v2.schema.json" with { type: "json" };
import bundleSchema from "../implementation/governance/schemas/reference-review-bundle.v1.schema.json" with { type: "json" };
import dependencyLockScanInputV2Schema from "../implementation/governance/schemas/reference-dependency-lock-scan-input.v2.schema.json" with { type: "json" };
...
import sourceIntegrationIndexV2Schema from "../implementation/governance/schemas/reference-source-integration-index.v2.schema.json" with { type: "json" };
import sourceIntegrationScanInputV2Schema from "../implementation/governance/schemas/reference-source-integration-scan-input.v2.schema.json" with { type: "json" };
import {
...
const RETROSPECTIVE_V2_WORK_PACKAGE_IDS = Object.freeze([
  "C04",
  "C06",
  "C07",
]);
...
const validateApplicabilityEvidenceSchema = (value) =>
  validateReferenceReviewSchema(
    value?.schemaVersion === "reference-applicability-evidence.v2"
      ? applicabilityEvidenceV2Schema
      : applicabilityEvidenceSchema,
    value,
  );
const validateDependencyLockScanInputV2Schema = (value) =>
  validateReferenceReviewSchema(
...
const validateSourceIntegrationIndexV2Schema = (value) =>
  validateReferenceReviewSchema(sourceIntegrationIndexV2Schema, value);
const validateSourceIntegrationScanInputV2Schema = (value) =>
  validateReferenceReviewSchema(
...
function bytesEqual(left, right) {
  const leftBytes =
    left instanceof Uint8Array ? left : null;
  const rightBytes =
    right instanceof Uint8Array ? right : null;
  return (
    leftBytes !== null &&
    rightBytes !== null &&
...
async function scanInputReferenceIds({
  ...
  switch (scanKind) {
    case "DEPENDENCY_LOCK_SCAN": {
      const isV2 = applicabilityVersion === "v2";
      if (
        (isV2
          ? !validateDependencyLockScanInputV2Schema(input) ||
            !keysExactly(input.inventory, ["files"])
          : input.schemaVersion !==
    ...
    case "SOURCE_INTEGRATION_SCAN": {
      ...
      if (
        (isV2
          ? reviewMode !== "RETROSPECTIVE_BACKFILL" ||
            !RETROSPECTIVE_V2_WORK_PACKAGE_IDS.includes(workPackageId) ||
            !validateSourceIntegrationScanInputV2Schema(input) ||
            !keysExactly(input.inventory, ["files"])
          : input.schemaVersion !==
      ...
      for (const file of input.inventory.files) {
        ...
        if (
          (isV2
            ? !validateSourceIntegrationIndexV2Schema(index)
            : !keysExactly(index, [
                "integrations",
                "schemaVersion",
                "workPackageId",
        ...
        for (const integration of index.integrations) {
          if (isV2) {
            let sourceBytes;
            try {
              sourceBytes = await readGitBytes({
                commit: sourceCommit,
            ...
            const sourceHash = await byteDigest(sourceBytes);
            const frozenSourceBytes = await readFrozenScanBytes({
              frozenEvidence,
              readGitBytes,
              pairs,
              path: integration.sourcePath,
              hash: integration.sourceCommitSha256,
            });
            const frozenImplementationEvidenceBytes =
              await readFrozenScanBytes({
                frozenEvidence,
                readGitBytes,
                pairs,
                path: integration.implementationEvidenceRef,
                hash: integration.implementationEvidenceSha256,
              });
            let sourceImplementationEvidenceBytes;
            try {
              sourceImplementationEvidenceBytes = await readGitBytes({
                commit: sourceCommit,
                path: integration.implementationEvidenceRef,
              });
            } catch {
            ...
            try {
              implementationEvidence = parseJsonBytes(
                frozenImplementationEvidenceBytes,
              );
            } catch {
              return null;
            }
            const matchingArtifacts = (
              implementationEvidence?.artifacts ?? []
            ).filter(
              (artifact) =>
                keysExactly(artifact, ["path", "sha256"]) &&
                artifact.path === integration.sourcePath &&
                artifact.sha256 === integration.sourceCommitSha256,
            );
            const reference = catalogReferenceFor({
              catalogByReferenceId,
              referenceName: integration.referenceName,
              officialSourceUri: integration.officialSourceUri,
            });
            if (
              sourceHash !== integration.sourceCommitSha256 ||
              frozenSourceBytes === null ||
              !bytesEqual(sourceBytes, frozenSourceBytes) ||
              frozenImplementationEvidenceBytes === null ||
              !bytesEqual(
                sourceImplementationEvidenceBytes,
                frozenImplementationEvidenceBytes,
              ) ||
              implementationEvidence?.work_package_id !==
                workPackageId ||
              !GIT_COMMIT.test(
                implementationEvidence?.verified_source_commit ?? "",
              ) ||
            ...
            sourceIntegrations.set(reference.referenceId, {
              identifier: integration.referenceName,
              officialSourceUri: integration.officialSourceUri,
...
async function validateApplicabilityEvidence({
  ...
  const applicabilityVersion =
    report?.schemaVersion === "reference-applicability-evidence.v2"
      ? "v2"
      : "v1";
  ...
  for (const scan of report.scans) {
    ...
    if (
      ...
      scan.methodVersion !== expectedMethodVersion ||
  ...
  const subjects = evidenceSubjectMap(frozenEvidence);
  const indexedSourcePaths = [...sourceIntegrations.values()]
    .map(({ sourcePath }) => sourcePath)
    .sort();
  if (applicabilityVersion === "v2") {
    if (
      !sortedUniqueStrings(indexedSourcePaths) ||
      referenceReviewDigests.canonicalize(sourceRoots) !==
        referenceReviewDigests.canonicalize(indexedSourcePaths) ||
      sourceExclusions.length !== 0 ||
      referenceReviewDigests.canonicalize(frozenSourcePaths) !==
        referenceReviewDigests.canonicalize(indexedSourcePaths)
    ) {
      return invalid();
    }
