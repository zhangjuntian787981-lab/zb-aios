...
const SENSITIVE_VALUE_TEXT =
  /\bbearer\s+[a-z0-9._~+\-/]+=*|\b[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-z0-9.-]+\.[a-z]{2,}\b/iu;
...
export function validateKimiK3TokenEstimateDiagnosticEvidenceV2({
  ...
  const schemaValidated =
    parsed.ok &&
    exactKeys(parsed.value, ["data"]) &&
    exactKeys(parsed.value?.data, ["total_tokens"]) &&
    Number.isSafeInteger(parsed.value.data.total_tokens);
  const semanticValidated =
    schemaValidated && parsed.value.data.total_tokens >= 0;
