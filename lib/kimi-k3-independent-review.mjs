...
// Current candidate paths. Historical readers must select V4, V3, then V2
// from the exact source commit rather than inheriting the runtime's version.
export const kimiK3ReviewMaterialPaths = kimiK3ReviewMaterialPathsV4;
...
export async function validateKimiK3ChatDiagnosticEvidenceV2({
  evidence,
  providerTransportSchemaPath,
  providerTransportSchemaSha256,
  canonicalOutputSchemaPath,
  ...
  let expected = null;
  try {
    expected = createKimiK3ChatDiagnosticEvidenceV2({
      ...input,
      providerTransportSchemaPath,
      providerTransportSchemaSha256,
      canonicalOutputSchemaPath,
  ...
  const valid =
    ...
    evidence.schemaVersion === CHAT_DIAGNOSTIC_SCHEMA_VERSION_V2 &&
    COMMIT.test(evidence.runtimeCommit ?? "") &&
    COMMIT.test(evidence.sourceCommit ?? "") &&
    COMMIT.test(evidence.sourceTree ?? "") &&
    SHA256.test(evidence.reviewBundleSha256 ?? "") &&
