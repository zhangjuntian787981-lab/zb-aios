...
const KIMI_K3_TOKEN_ESTIMATE_APPLICATION_TIMEOUT_MAX_MS = 120_000;
const KIMI_K3_CHAT_APPLICATION_TIMEOUT_MAX_MS = 1_800_000;
...
export const kimiK3ReviewMaterialPathsV6 = Object.freeze({
  ...kimiK3ReviewMaterialPathsV5,
  receiptSchema:
    "implementation/governance/schemas/independent-model-review-receipt.v8.schema.json",
});

export const kimiK3ReviewMaterialPathsV7 = Object.freeze({
...
// Current candidate paths. Historical readers must select V7 through V2
// from the exact source commit rather than inheriting the runtime's version.
export const kimiK3ReviewMaterialPaths = kimiK3ReviewMaterialPathsV7;
...
export const kimiK3ReviewMaterialGovernancePathsV6 = Object.freeze(
  [
    ...new Set([
...
export const kimiK3ReviewMaterialGovernancePathsV7 = Object.freeze(
  [
    ...new Set([
...
const transientSensitiveResponseBytes = new WeakMap();
...
function transportFailure(
  ...
  if (sensitiveResponseBytes instanceof Uint8Array) {
    transientSensitiveResponseBytes.set(
      failure,
      Buffer.from(sensitiveResponseBytes),
    );
  }
  return failure;
}

export function consumeKimiK3TransientSensitiveResponseBytes(failure) {
  const bytes = transientSensitiveResponseBytes.get(failure) ?? null;
  transientSensitiveResponseBytes.delete(failure);
  return bytes;
}
...
const KIMI_K3_CHAT_HIGH_CONFIDENCE_SENSITIVE_TEXT =
  /\b(?:sk-[a-z0-9_-]{16,}|github_pat_[a-z0-9_]{20,}|gh[pousr]_[a-z0-9]{20,}|AKIA[A-Z0-9]{16}|eyJ[a-z0-9_-]{8,}\.[a-z0-9_-]{8,}\.[a-z0-9_-]{8,})\b/iu;
const KIMI_K3_CHAT_PRIVATE_KEY_BLOCK =
  /-----BEGIN ((?:[A-Z0-9]+ )*PRIVATE KEY)-----\r?\n(?:[A-Za-z0-9+/]{16,}={0,2}\r?\n)+-----END \1-----/u;
const KIMI_K3_CHAT_EMAIL =
  /\b[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@([a-z0-9.-]+\.[a-z]{2,})\b/giu;
const KIMI_K3_CHAT_AUTHORIZATION_VALUE =
  /(?:\bauthorization["']?\s*:\s*["']?)?\b(bearer|basic)\s+([a-z0-9._~+\-/=]{12,})\b/giu;
const KIMI_K3_CHAT_COOKIE_VALUE =
  /\b(?:set-cookie|cookie)["']?\s*:\s*["']?([^\s;=]+)=([^\s;,"']+)/giu;
const KIMI_K3_CHAT_SECRET_ASSIGNMENT =
  /(?:^|[\s{[(,])["']?(?:api[_-]?key|client[_-]?secret|password|access[_-]?token|refresh[_-]?token)["']?\s*([:=])\s*(?:"([^"]{1,4096})"|'([^']{1,4096})'|([^\s,;]{1,4096}))/gimu;
const KIMI_K3_CHAT_SAFE_PLACEHOLDERS = new Set([
  "absent",
  "example",
  "masked",
  "none",
...
function isExplicitKimiK3ChatSecretPlaceholder(rawValue) {
  ...
  return (
    ...
    /^\*+$/u.test(normalized) ||
    KIMI_K3_CHAT_SAFE_PLACEHOLDERS.has(stripped.toLowerCase())
  );
}

function expandJsonUnicodeEscapes(text) {
  let expanded = text;
...
function authorizationValueIsHighConfidence(kind, value) {
  if (isExplicitKimiK3ChatSecretPlaceholder(value)) return false;
  if (kind.toLowerCase() === "bearer") {
    return (
      value.length >= 20 && /^[a-z0-9._~+\-/=]+$/iu.test(value)
    );
  }
  try {
    const decoded = Buffer.from(value, "base64").toString("utf8");
    return decoded.includes(":") && /^[\u0020-\u007e]+$/u.test(decoded);
  } catch {
    return false;
  }
}

function textContainsKimiK3ChatHighConfidenceSensitiveMaterial(text) {
  if (
    KIMI_K3_CHAT_PRIVATE_KEY_BLOCK.test(text) ||
    KIMI_K3_CHAT_HIGH_CONFIDENCE_SENSITIVE_TEXT.test(text)
  ) {
    return true;
  }
  for (const match of text.matchAll(KIMI_K3_CHAT_AUTHORIZATION_VALUE)) {
    if (authorizationValueIsHighConfidence(match[1], match[2])) return true;
  }
  for (const match of text.matchAll(KIMI_K3_CHAT_COOKIE_VALUE)) {
    if (!isExplicitKimiK3ChatSecretPlaceholder(match[2])) return true;
  }
  for (const match of text.matchAll(KIMI_K3_CHAT_SECRET_ASSIGNMENT)) {
    const operator = match[1];
    const quoted = match[2] ?? match[3] ?? null;
    const rawValue = quoted ?? match[4];
    const value = rawValue.replace(/^[<${*]+|[>}*]+$/gu, "");
    if (
      !isExplicitKimiK3ChatSecretPlaceholder(rawValue) &&
      (operator === "=" || quoted !== null || value.length >= 12)
    ) {
      return true;
    }
  }
  for (const match of text.matchAll(KIMI_K3_CHAT_EMAIL)) {
...
function activeCredentialRepresentations(credential) {
...
function textContainsExactCredentialRepresentation(
  text,
  representations,
  lowercaseHexRepresentation,
) {
  const expanded = expandJsonUnicodeEscapes(text);
  return (
    representations.some((representation) =>
      expanded.includes(representation),
    ) ||
    (typeof lowercaseHexRepresentation === "string" &&
      expanded.toLowerCase().includes(lowercaseHexRepresentation))
  );
}

function stringValuesContainSplitExactCredential(
  stringValues,
  representations,
  lowercaseHexRepresentation,
) {
  const minimumFragmentLength = 8;
  const candidates = [
    ...representations
      .filter(
        (representation) =>
          representation !== lowercaseHexRepresentation &&
          representation !== lowercaseHexRepresentation?.toUpperCase(),
      )
      .map((representation) => ({ representation, caseInsensitive: false })),
    ...(typeof lowercaseHexRepresentation === "string"
      ? [{
          representation: lowercaseHexRepresentation,
  ...
  for (const { representation, caseInsensitive } of candidates) {
    if (representation.length < minimumFragmentLength * 2) continue;
    const values = caseInsensitive
      ? stringValues.map((value) => value.toLowerCase())
      : stringValues;
    const exactRepresentation = caseInsensitive
      ? representation.toLowerCase()
      : representation;
    const reachableOffsets = new Set([0]);
    for (let offset = 0; offset < exactRepresentation.length; offset += 1) {
      if (!reachableOffsets.has(offset)) continue;
      for (
        let end = offset + minimumFragmentLength;
        end <= exactRepresentation.length;
        end += 1
      ) {
        const fragment = exactRepresentation.slice(offset, end);
        if (values.some((value) => value.includes(fragment))) {
          reachableOffsets.add(end);
        }
      }
    }
    if (reachableOffsets.has(exactRepresentation.length)) return true;
  }
  return false;
}

function decodedJsonContainsExactCredential(value, credential) {
  const representations = activeCredentialRepresentations(credential);
  const lowercaseHexRepresentation =
    typeof credential === "string" && credential.length > 0
      ? Buffer.from(credential, "utf8").toString("hex")
      : null;
  if (representations.length === 0) return false;
  const narrativeText = [
    typeof value?.reviewSummary === "string" ? value.reviewSummary : "",
  ...
  if (
    textContainsExactCredentialRepresentation(
      narrativeText,
      representations,
      lowercaseHexRepresentation,
    )
  ...
  return stringValuesContainSplitExactCredential(
    stringValues,
    representations,
    lowercaseHexRepresentation,
  );
...
function decodedJsonContainsKimiK3ChatHighConfidenceSensitiveMaterial(value) {
  const pending = [value];
  let inspectedValues = 0;
  while (pending.length > 0) {
    inspectedValues += 1;
    ...
    const current = pending.pop();
    if (typeof current === "string") {
      const parsed = parseJsonValueBytes(Buffer.from(current, "utf8"));
      if (
        parsed.ok &&
        parsed.value !== null &&
      ...
      if (textContainsKimiK3ChatHighConfidenceSensitiveMaterial(current)) {
        return true;
      }
      continue;
    }
      ...
      for (const [key, entry] of Object.entries(current)) {
        if (typeof entry === "string") {
          ...
          if (
            ...
            !isExplicitKimiK3ChatSecretPlaceholder(entry)
          ) {
            return true;
          }
          if (normalizedKey === "authorization") {
            const authorization = /^(bearer|basic)\s+(.+)$/iu.exec(entry);
            if (
              authorization !== null &&
              authorizationValueIsHighConfidence(
                authorization[1],
                authorization[2],
              )
            ) {
          ...
          if (
            ["cookie", "setcookie"].includes(normalizedKey) &&
            /^[^\s;=]+=[^\s;]+$/u.test(entry)
          ) {
            const separatorIndex = entry.indexOf("=");
            if (
              !isExplicitKimiK3ChatSecretPlaceholder(
...
function bytesContainExactActiveCredential(bytes, credential) {
  if (!(bytes instanceof Uint8Array)) return false;
  const representations = activeCredentialRepresentations(credential);
  const lowercaseHexRepresentation =
    typeof credential === "string" && credential.length > 0
  ...
  if (
    representations.some((representation) =>
      bytesContainCredential(bytes, representation),
    )
  ) {
    return true;
  }
  ...
  const unicodeExpanded = expandJsonUnicodeEscapes(text);
  if (
    textContainsExactCredentialRepresentation(
      unicodeExpanded,
      representations,
  ...
  if (decodedJsonContainsExactCredential(parsed.value, credential)) return true;
  for (const choice of Array.isArray(parsed.value?.choices)
    ? parsed.value.choices
    : []) {
    const content = choice?.message?.content;
    if (typeof content !== "string") continue;
    const decodedContent = parseJsonValueBytes(Buffer.from(content, "utf8"));
    if (
      decodedContent.ok &&
      decodedJsonContainsExactCredential(decodedContent.value, credential)
    ) {
      return true;
    }
  }
...
function bytesContainKimiK3ChatHighConfidenceSensitiveMaterial(bytes) {
  if (!(bytes instanceof Uint8Array)) return false;
  const text = strictUtf8(bytes);
  if (text === null) return true;
  const parsed = parseJsonValueBytes(bytes);
  if (!parsed.ok) {
    const unicodeExpanded = expandJsonUnicodeEscapes(text);
    return (
      textContainsKimiK3ChatHighConfidenceSensitiveMaterial(text) ||
      textContainsKimiK3ChatHighConfidenceSensitiveMaterial(unicodeExpanded)
    );
  }

  const outer = structuredClone(parsed.value);
  const modelVisibleText = [];
  for (const choice of Array.isArray(outer?.choices) ? outer.choices : []) {
    const message = choice?.message;
    if (!message || typeof message !== "object") continue;
    for (const key of ["content", "reasoning_content"]) {
      if (typeof message[key] !== "string") continue;
      modelVisibleText.push(message[key]);
      message[key] = "";
    }
  }
  return (
    decodedJsonContainsSensitiveMaterial(outer, null) ||
    modelVisibleText.some((value) =>
      decodedJsonContainsKimiK3ChatHighConfidenceSensitiveMaterial(value),
    )
  );
}

function bytesContainKimiK3ChatSensitiveMaterial(bytes, credential) {
  return (
    bytesContainExactActiveCredential(bytes, credential) ||
    bytesContainKimiK3ChatHighConfidenceSensitiveMaterial(bytes)
  );
}
...
export function classifyKimiK3V6ContractPresence({
  v4Presence,
  v5AdditivePresence,
  v6AdditivePresence,
}) {
  if (
    !Array.isArray(v6AdditivePresence) ||
    v6AdditivePresence.length === 0 ||
    !v6AdditivePresence.every((value) => typeof value === "boolean")
  ) {
    throw new TypeError("Kimi K3 v6 contract presence is invalid.");
  }
  const v5Classification = classifyKimiK3V5ContractPresence({
    v4Presence,
    v5AdditivePresence,
  });
  if (!v6AdditivePresence.some(Boolean)) return v5Classification;
  return v5Classification === "K3_V5_COMPLETE" &&
    v6AdditivePresence.every(Boolean)
    ? "K3_V6_COMPLETE"
    : "K3_V6_INCOMPLETE";
}

export function classifyKimiK3V7ContractPresence({
  v4Presence,
  v5AdditivePresence,
  v6AdditivePresence,
  v7AdditivePresence,
  ...
  if (
    !Array.isArray(v7AdditivePresence) ||
    v7AdditivePresence.length === 0 ||
    !v7AdditivePresence.every((value) => typeof value === "boolean")
  ) {
    throw new TypeError("Kimi K3 v7 contract presence is invalid.");
  }
  const v6Classification = classifyKimiK3V6ContractPresence({
    v4Presence,
    v5AdditivePresence,
    v6AdditivePresence,
  });
  if (!v7AdditivePresence.some(Boolean)) return v6Classification;
  return v6Classification === "K3_V6_COMPLETE" &&
    v7AdditivePresence.every(Boolean)
    ? "K3_V7_COMPLETE"
    : "K3_V7_INCOMPLETE";
}
...
async function postBoundJson({
  ...
  timeoutMs,
  maximumTimeoutMs,
  retainSensitiveResponseForDiagnostic = false,
}) {
  ...
  try {
    result = await postBoundJsonWithDispatcher({
      ...
      retainSensitiveResponseForDiagnostic,
    });
  } catch {
...
async function postBoundJsonWithDispatcher({
  ...
  if (bytesContainExactActiveCredential(responseBytes, apiKey)) {
    const failure = transportFailure(
      ...
      null,
      retainSensitiveResponseForDiagnostic ? responseBytes : null,
    );
...
export async function executeKimiK3TokenEstimate({
  ...
  const transport = await postBoundJson({
    ...
    dispatcherFactory,
    timeoutMs,
    maximumTimeoutMs: KIMI_K3_TOKEN_ESTIMATE_APPLICATION_TIMEOUT_MAX_MS,
  });
  if (!transport.ok) return transport;
  if (bytesContainSensitiveMaterial(transport.responseBytes, apiKey)) {
    return transportFailure(
      ["KIMI_K3_RESPONSE_CREDENTIAL_ECHOED"],
      1,
      {
...
export async function executeKimiK3ChatCompletion({
  ...
  fetchImpl,
  dispatcherFactory = null,
  timeoutMs = KIMI_K3_CHAT_APPLICATION_TIMEOUT_MAX_MS,
}) {
  const configValidation = await validateMoonshotKimiK3Config(config);
  ...
  const transport = await postBoundJson({
    ...
    dispatcherFactory,
    timeoutMs,
    maximumTimeoutMs: KIMI_K3_CHAT_APPLICATION_TIMEOUT_MAX_MS,
    retainSensitiveResponseForDiagnostic: true,
  });
  if (!transport.ok) return transport;
  const contentTypeValid =
    transport.metadataValidity.contentType &&
  ...
  if (!validation.ok) {
    const sensitiveResponse = validation.reasonCodes.some((reasonCode) =>
      [
        "KIMI_K3_RESPONSE_CREDENTIAL_ECHOED",
    ...
    const failure = transportFailure(
      ...
      sensitiveResponse ? null : transport.responseBytes,
      sensitiveResponse ? transport.responseBytes : null,
    );
    if (sensitiveResponse) transport.responseBytes.fill(0);
    return failure;
  }
...
const CHAT_DIAGNOSTIC_SCHEMA_VERSION_V4 =
...
function chatDiagnosticResponseCanBePersisted({
  ...
  return (
    ...
    !bytesContainKimiK3ChatSensitiveMaterial(
      responseBytes,
      sensitiveCredential,
    )
  );
...
async function analyzeKimiK3ChatDiagnostic({
  ...
  allowSensitiveMaterialReason = false,
}) {
  ...
  const exactCredentialResponse =
    responseBytesMatch &&
    bytesContainExactActiveCredential(responseBytes, sensitiveCredential);
  if (
    exactCredentialResponse ||
    exactReasonCodes.includes("KIMI_K3_RESPONSE_CREDENTIAL_ECHOED")
  ) {
    return result({
      valid:
        (allowSensitiveMaterialReason
          ? exactCredentialResponse
  ...
  const highConfidenceSensitiveResponse =
    responseBytesMatch &&
    bytesContainKimiK3ChatHighConfidenceSensitiveMaterial(responseBytes);
  if (
    allowSensitiveMaterialReason &&
    exactReasonCodes.includes(
      "KIMI_K3_RESPONSE_SENSITIVE_MATERIAL_DETECTED",
    )
  ) {
    let progress;
    let sensitiveMaterialProved = false;
    if (highConfidenceSensitiveResponse) {
      ...
      sensitiveMaterialProved =
        responseAnalysis.ok === false &&
        responseAnalysis.reasonCodes.includes(
          "KIMI_K3_RESPONSE_SENSITIVE_MATERIAL_DETECTED",
        );
    ...
    return result({
      valid:
        sensitiveMaterialProved &&
        diagnostic.failureStage === "SCHEMA_OR_SEMANTIC_VALIDATION",
      persistResponse: false,
      failureStage: "SEMANTIC_VALIDATION",
      expectedReasonCodes: [
        "KIMI_K3_RESPONSE_SENSITIVE_MATERIAL_DETECTED",
...
function createKimiK3ChatDiagnosticEvidenceV4(input) {
  const evidence = createKimiK3ChatDiagnosticEvidenceV2(input);
  evidence.schemaVersion = CHAT_DIAGNOSTIC_SCHEMA_VERSION_V4;
  evidence.evidenceSha256 = chatDiagnosticEvidenceDigest(evidence);
  return evidence;
}
...
async function validateKimiK3ChatDiagnosticEvidenceVersion(
  ...
  try {
    const createEvidence =
      expectedSchemaVersion === CHAT_DIAGNOSTIC_SCHEMA_VERSION_V4
        ? createKimiK3ChatDiagnosticEvidenceV4
  ...
  const artifactBound =
    evidence?.responseArtifact === "chat-diagnostic-response.bin";
  const unmaterializedSensitiveArtifact =
    expectedSchemaVersion === CHAT_DIAGNOSTIC_SCHEMA_VERSION_V4 &&
    evidence?.responseArtifact === null &&
    Array.isArray(evidence?.reasonCodes) &&
    ...
    [
      "KIMI_K3_RESPONSE_CREDENTIAL_ECHOED",
      "KIMI_K3_RESPONSE_SENSITIVE_MATERIAL_DETECTED",
    ].includes(evidence.reasonCodes[0]);
  const artifactBytesValid = artifactBound
    ? responseArtifactBytes instanceof Uint8Array &&
      responseArtifactBytes.byteLength === evidence.responseBodyByteLength &&
      sha256Bytes(responseArtifactBytes) === evidence.responseBodySha256 &&
    ...
    : unmaterializedSensitiveArtifact
      ? responseArtifactBytes instanceof Uint8Array &&
        responseArtifactBytes.byteLength === evidence.responseBodyByteLength &&
        sha256Bytes(responseArtifactBytes) === evidence.responseBodySha256
      : responseArtifactBytes === null;
  ...
  return {
    ...
    reasonCodes: valid
      ...
      : [
          expectedSchemaVersion === CHAT_DIAGNOSTIC_SCHEMA_VERSION_V4
            ? "KIMI_K3_CHAT_DIAGNOSTIC_EVIDENCE_V4_INVALID"
            : expectedSchemaVersion === CHAT_DIAGNOSTIC_SCHEMA_VERSION_V3
...
export async function validateKimiK3ChatDiagnosticEvidenceV4(input) {
  try {
    return await validateKimiK3ChatDiagnosticEvidenceVersion(input, {
      expectedSchemaVersion: CHAT_DIAGNOSTIC_SCHEMA_VERSION_V4,
      allowedStages: CHAT_DIAGNOSTIC_STAGES_V3,
      allowPostResponseCleanup: true,
      allowSensitiveMaterialReason: true,
    });
  } catch {
    return {
      ok: false,
      reasonCodes: ["KIMI_K3_CHAT_DIAGNOSTIC_EVIDENCE_V4_INVALID"],
    };
  }
}
...
export async function createKimiK3ChatDiagnosticArtifactsV4({
  responseBytes = null,
  resolveSourceCommitBytes,
  ...input
}) {
  ...
  if (!sourceBindingsValid) {
    throw new TypeError("Kimi K3 Chat diagnostic evidence v4 is invalid.");
  }
  const analysis = await analyzeKimiK3ChatDiagnostic({
    ...input,
    outputSchemaSha256: input.providerTransportSchemaSha256,
  ...
  const responseArtifact = analysis.persistResponse
    ? "chat-diagnostic-response.bin"
    : null;
  const evidence = createKimiK3ChatDiagnosticEvidenceV4({
    ...input,
    analysis,
    responseArtifact,
  });
  const validation = await validateKimiK3ChatDiagnosticEvidenceV4({
    ...input,
    evidence,
    responseArtifactBytes:
      analysis.persistResponse ||
      analysis.reasonCodes.includes("KIMI_K3_RESPONSE_CREDENTIAL_ECHOED") ||
      analysis.reasonCodes.includes(
        "KIMI_K3_RESPONSE_SENSITIVE_MATERIAL_DETECTED",
      )
        ? responseBytes
        : null,
...
async function analyzeKimiK3ChatResponse({
  ...
  if (bytesContainKimiK3ChatHighConfidenceSensitiveMaterial(responseBytes)) {
    return {
      ok: false,
      reasonCodes: ["KIMI_K3_RESPONSE_SENSITIVE_MATERIAL_DETECTED"],
      failureStage: "SEMANTIC_VALIDATION",
      progress: {
        jsonParsed: true,
        protocolValidated: true,
