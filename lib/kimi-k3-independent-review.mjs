...
// Current candidate paths. Historical readers must select V3 then V2 from Git.
export const kimiK3ReviewMaterialPaths = kimiK3ReviewMaterialPathsV3;
...
function bytesContainSensitiveMaterial(bytes, credential) {
  if (!(bytes instanceof Uint8Array)) return false;
  if (bytesContainCredential(bytes, credential)) return true;
  const text = strictUtf8(bytes);
  if (text === null) return true;
  return (
    /"(?:authorization|cookie|set-cookie|api[_-]?key|access[_-]?token|refresh[_-]?token|token|credential|secret|password|email|user[_-]?email|account[_-]?id|user[_-]?id|tenant[_-]?id|request[_-]?headers)"\s*:/iu.test(
      text,
    ) ||
    /\bbearer\s+[a-z0-9._~+\-/]+=*/iu.test(text) ||
...
export async function executeKimiK3TokenEstimate({
  ...
  const diagnostic = {
    ...
    jsonParsed: failureStage === null && jsonParsed,
    schemaValidated: failureStage === null && schemaValidated,
    semanticValidated: failureStage === null && semanticValidated,
    failureStage,
    reasonCode,
  };
...
export async function executeKimiK3ChatCompletion({
  ...
  if (
    ...
    return transportFailure(
      [...configValidation.reasonCodes, "KIMI_K3_CHAT_REQUEST_BINDING_MISMATCH"],
      0,
      emptyTransportDiagnostic({
        failureStage: "REQUEST_BINDING",
