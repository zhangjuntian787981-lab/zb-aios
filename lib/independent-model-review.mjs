...
const TAP_SUMMARY_KEYS = Object.freeze([
  ...
  "todo",
  "skippedTestNames",
  "skippedTestSetSha256",
]);
const TAP_EXPECTATION_KEYS = Object.freeze([
  "format",
  "reporter",
  "tests",
  "pass",
...
const TAP_COUNT_FIELDS = Object.freeze([
  "tests",
  "pass",
  "fail",
  "cancelled",
...
function validTapTestNames(values) {
  return (
    Array.isArray(values) &&
    values.length <= 4096 &&
    values.every(
    ...
    unique(values) &&
    canonicalizeProjectJson(values) ===
      canonicalizeProjectJson(sortedUtf8Strings(values))
  );
}

async function validTapExpectation(expectation) {
  return (
    keysExactly(expectation, TAP_EXPECTATION_KEYS) &&
    expectation.format === "NODE_TEST_TAP" &&
    expectation.reporter === "TAP" &&
    TAP_COUNT_FIELDS.every(
      (field) =>
        Number.isInteger(expectation[field]) &&
        expectation[field] >= 0 &&
        expectation[field] <= 1_000_000,
    ) &&
    expectation.tests ===
      expectation.pass +
        expectation.fail +
        ...
        expectation.todo &&
    expectation.fail === 0 &&
    expectation.cancelled === 0 &&
    expectation.todo === 0 &&
    validTapTestNames(expectation.allowedSkippedTestNames) &&
    expectation.allowedSkippedTestNames.length === expectation.skipped &&
    SHA256.test(expectation.allowedSkippedTestSetSha256 ?? "") &&
    expectation.allowedSkippedTestSetSha256 ===
      (await sha256ProjectValue(expectation.allowedSkippedTestNames))
  );
}

function commandRequiresTapExpectation(command) {
  return (
    (command?.executable === "NODE" &&
      Array.isArray(command.args) &&
      command.args.includes("--test")) ||
    (command?.executable === "NPM" &&
      canonicalizeProjectJson(command.args) ===
        canonicalizeProjectJson(["test"]))
  );
}

function commandRequestsTapReporter(command) {
  return (
    (command?.executable === "NODE" &&
      command.args?.[0] === "--test" &&
      command.args?.[1] === "--test-reporter=tap") ||
    (command?.executable === "NPM" &&
      canonicalizeProjectJson(command.args) ===
        canonicalizeProjectJson(["test"]))
  );
...
async function validTestPlanCommand(command) {
  const requiresTap = commandRequiresTapExpectation(command);
  if (
    !keysExactly(
      command,
      requiresTap
        ...
        : ["commandId", "executable", "args", "timeoutMs"],
    ) ||
  ...
  return (
    !requiresTap ||
    (commandRequestsTapReporter(command) &&
      (await validTapExpectation(command.testExpectation)))
  );
}

function parseTapStructuralLine(line) {
  const body = line.trimStart();
  if (
    body.startsWith("Bail out!") ||
    /^TAP version(?:[ \t]|$)/u.test(body) ||
    /^pragma(?:[ \t]|$)/u.test(body) ||
    /^# (?:tests|suites|pass|fail|cancelled|skipped|todo|duration_ms)(?:[ \t]|$)/u.test(
      body,
    )
  ) {
    return { kind: "INVALID" };
  ...
  const isStructural =
    body.startsWith("# Subtest:") ||
    /^(?:ok|not ok)(?:[ \t]|$)/u.test(body) ||
    body.startsWith("1..");
  if (!isStructural) return null;

  const indentation = line.slice(0, line.length - body.length);
  if (!/^ *$/u.test(indentation) || indentation.length % 4 !== 0) {
    return { kind: "INVALID" };
  }
  const indent = indentation.length;
  if (body.startsWith("# Subtest:")) {
    const match = /^# Subtest: (.+)$/u.exec(body);
    const name = match?.[1];
    return typeof name === "string" &&
      name.length <= 512 &&
      !/[\u0000-\u001f]/u.test(name)
      ? { kind: "SUBTEST", indent, name }
      : { kind: "INVALID" };
  }
  const plan = /^1\.\.([0-9]+)$/u.exec(body);
  if (plan !== null) {
    const tests = Number(plan[1]);
    return Number.isSafeInteger(tests) && tests <= 1_000_000
      ? { kind: "PLAN", indent, tests }
      : { kind: "INVALID" };
  }
  const resultLine = /^(ok|not ok) ([0-9]+) - (.+)$/u.exec(body);
  if (resultLine === null) return { kind: "INVALID" };
  const number = Number(resultLine[2]);
  return Number.isSafeInteger(number) && number <= 1_000_000
    ? {
        kind: "RESULT",
        indent,
        status: resultLine[1],
        number,
        remainder: resultLine[3],
      }
    : { kind: "INVALID" };
}

function parseTapResult(resultLine, expectedNumber, subtestName) {
  if (resultLine.number !== expectedNumber) return null;
  let directive = null;
  if (resultLine.remainder !== subtestName) {
    const suffix = resultLine.remainder.slice(subtestName.length);
    if (!resultLine.remainder.startsWith(subtestName)) return null;
    const match = /^ # (SKIP|TODO|CANCELLED)(?: .+)?$/u.exec(suffix);
    if (match === null) return null;
    directive = match[1];
  }
  if (directive === "SKIP" && resultLine.status !== "ok") return null;
  return {
    name: subtestName,
    outcome:
      directive === "SKIP"
        ...
        : directive === "TODO"
          ...
          : directive === "CANCELLED"
            ? "cancelled"
            : resultLine.status === "ok"
              ? "pass"
              : "fail",
  };
}

function parseTapScope(lines, startIndex, indent) {
  const points = [];
  let index = startIndex;
  let expectedNumber = 1;
  while (index < lines.length) {
    const structural = parseTapStructuralLine(lines[index]);
    if (structural === null) {
      index += 1;
      continue;
    }
    if (structural.kind === "INVALID" || structural.indent !== indent) {
      return null;
    }
    if (structural.kind === "PLAN") {
      return structural.tests === expectedNumber - 1
        ? { nextIndex: index + 1, points }
        : null;
    }
    if (structural.kind !== "SUBTEST") return null;

    const subtestName = structural.name;
    const nestedPoints = [];
    let nestedScopeSeen = false;
    let resultSeen = false;
    index += 1;
    while (index < lines.length) {
      const child = parseTapStructuralLine(lines[index]);
      if (child === null) {
        index += 1;
        continue;
      }
      if (child.kind === "INVALID") return null;
      if (
        child.indent === indent + 4 &&
        child.kind === "SUBTEST" &&
        !nestedScopeSeen
      ) {
        const nestedScope = parseTapScope(lines, index, indent + 4);
        if (nestedScope === null) return null;
        nestedPoints.push(...nestedScope.points);
        nestedScopeSeen = true;
        index = nestedScope.nextIndex;
        continue;
      }
      if (child.indent === indent && child.kind === "RESULT") {
        const point = parseTapResult(
          child,
          expectedNumber,
          subtestName,
        );
        if (point === null) return null;
        points.push(...nestedPoints, point);
        expectedNumber += 1;
        resultSeen = true;
        index += 1;
        break;
    ...
    if (!resultSeen) return null;
  }
  return null;
}

export async function parseIndependentReviewTapSummary(stdoutBytes) {
  if (!(stdoutBytes instanceof Uint8Array)) return null;
  let text;
  try {
  ...
  if (lines.some((line) => line.trimStart().startsWith("Bail out!"))) {
    return null;
  }
  const tapHeaderIndexes = lines
    .map((line, index) => (line === "TAP version 13" ? index : null))
    .filter((value) => value !== null);
  if (
    tapHeaderIndexes.length !== 1 ||
    lines
      .slice(0, tapHeaderIndexes[0])
      .some((line) => parseTapStructuralLine(line) !== null)
  ) {
    return null;
  }
  const parsed =
    parseTapScope(lines, tapHeaderIndexes[0] + 1, 0);
  const footer =
    parsed === null ? [] : lines.slice(parsed.nextIndex);
  if (
    tapHeaderIndexes.length !== 1 ||
    parsed === null ||
    footer.length !== 8 ||
    !/^# tests [0-9]+$/u.test(footer[0] ?? "") ||
    !/^# suites [0-9]+$/u.test(footer[1] ?? "") ||
    !/^# pass [0-9]+$/u.test(footer[2] ?? "") ||
    !/^# fail [0-9]+$/u.test(footer[3] ?? "") ||
    !/^# cancelled [0-9]+$/u.test(footer[4] ?? "") ||
    !/^# skipped [0-9]+$/u.test(footer[5] ?? "") ||
    !/^# todo [0-9]+$/u.test(footer[6] ?? "") ||
    !/^# duration_ms [0-9]+(?:\.[0-9]+)?$/u.test(footer[7] ?? "")
  ) {
    return null;
  }
  const counts = {};
  const footerIndexByField = {
    tests: 0,
    pass: 2,
    fail: 3,
    cancelled: 4,
  ...
  for (const field of TAP_COUNT_FIELDS) {
    const match = new RegExp(`^# ${field} ([0-9]+)$`, "u").exec(
      footer[footerIndexByField[field]],
    );
    if (match === null) return null;
    ...
    if (
      ...
      counts[field] < 0 ||
      counts[field] > 1_000_000
    ) {
      return null;
    }
  }
  const derivedCounts = {
    tests: parsed.points.length,
    pass: 0,
    fail: 0,
    cancelled: 0,
    skipped: 0,
  ...
  for (const point of parsed.points) {
    derivedCounts[point.outcome] += 1;
  }
  const skippedTestNames = sortedUtf8Strings(
    parsed.points
      .filter((point) => point.outcome === "skipped")
      .map((point) => point.name),
  );
  if (
    TAP_COUNT_FIELDS.some(
      (field) => counts[field] !== derivedCounts[field],
    ) ||
    counts.tests !==
      counts.pass +
        counts.fail +
        ...
        counts.skipped +
        counts.todo ||
    skippedTestNames.length !== counts.skipped ||
    !validTapTestNames(skippedTestNames)
  ) {
    return null;
  }
  return {
    ...
    ...counts,
    skippedTestNames,
    skippedTestSetSha256: await sha256ProjectValue(skippedTestNames),
  };
}

export async function tapSummaryMatchesExpectation(summary, expectation) {
  return (
    keysExactly(summary, TAP_SUMMARY_KEYS) &&
    (await validTapExpectation(expectation)) &&
    summary.format === expectation.format &&
    TAP_COUNT_FIELDS.every(
      (field) => summary[field] === expectation[field],
    ) &&
    validTapTestNames(summary.skippedTestNames) &&
    summary.skippedTestSetSha256 ===
      (await sha256ProjectValue(summary.skippedTestNames)) &&
    canonicalizeProjectJson(summary.skippedTestNames) ===
      canonicalizeProjectJson(expectation.allowedSkippedTestNames) &&
    summary.skippedTestSetSha256 ===
      expectation.allowedSkippedTestSetSha256
  );
}

export async function assertIndependentReviewGitCollectionUnchanged(
  frozenRefTips,
  currentRefTips,
) {
  if (
    !Array.isArray(frozenRefTips) ||
    !Array.isArray(currentRefTips) ||
    canonicalizeProjectJson(frozenRefTips) !==
      canonicalizeProjectJson(currentRefTips)
  ) {
    throw new TypeError(
      "Independent review ref-tip set changed during collection.",
    );
  }
}

export async function validateIndependentReviewTestPlan(plan) {
  if (
    !keysExactly(plan, [
      "schemaVersion",
      "planId",
  ...
  const commandResults = await Promise.all(
    plan.commands.map(validTestPlanCommand),
  );
...
export async function validateIndependentReviewTestEvidenceClosure({
  ...
  if (
    !(await validateIndependentReviewTestPlan(plan)) ||
  ...
  let acceptedGitCollectionDescriptor = null;
  try {
    for (let index = 0; index < plan.commands.length; index += 1) {
      ...
      if (
        ...
        !SHA256.test(
          attestation.runtimeBinding.systemToolchainSha256 ?? "",
        ) ||
        ...
        !keysExactly(attestation.executionSource.gitHistory, [
          "mode",
          "sourceCommit",
          "sourceTree",
          "refTipCount",
        ...
        attestation.executionSource.gitHistory.mode !==
          "READ_ONLY_ALL_REF_REACHABLE_OBJECT_SNAPSHOT" ||
        attestation.executionSource.gitHistory.sourceCommit !==
          bundle.source.sourceCommit ||
        attestation.executionSource.gitHistory.sourceTree !==
          bundle.source.tree ||
        !Number.isInteger(
          attestation.executionSource.gitHistory.refTipCount,
        ) ||
        attestation.executionSource.gitHistory.refTipCount < 1 ||
        !SHA256.test(
          attestation.executionSource.gitHistory.refTipSetSha256 ?? "",
        ) ||
        !Number.isInteger(
          attestation.executionSource.gitHistory.reachableObjectCount,
        ) ||
        attestation.executionSource.gitHistory.reachableObjectCount < 1 ||
        !SHA256.test(
          attestation.executionSource.gitHistory
            .reachableObjectSetSha256 ?? "",
        ) ||
        !SHA256.test(
          attestation.executionSource.gitHistory.pointerSha256 ?? "",
        ) ||
        !SHA256.test(
          attestation.executionSource.gitHistory
            .beforeManifestSha256 ?? "",
        ) ||
        attestation.executionSource.gitHistory.beforeManifestSha256 !==
          attestation.executionSource.gitHistory.afterManifestSha256 ||
        attestation.executionSource.gitHistory.unchanged !== true ||
        attestation.executionSource.gitHistory.metadataWritable !== false ||
        ...
        attestation.executionSource.sandbox.gitMetadataPresent !== true ||
        attestation.executionSource.sandbox.gitMetadataWritable !== false ||
        ...
        !evidence.toolVersions.includes(
          `system-toolchain=${attestation.runtimeBinding.systemToolchainSha256}`,
        ) ||
        ...
        (Object.hasOwn(command, "testExpectation")
          ? !(await tapSummaryMatchesExpectation(
              attestation.testSummary,
              command.testExpectation,
            ))
          : attestation.testSummary !== null) ||
      ...
      if (
        acceptedGitCollectionDescriptor !== null &&
        canonicalizeProjectJson(acceptedGitCollectionDescriptor) !==
          canonicalizeProjectJson(gitCollectionDescriptor)
      ) {
        return fail();
      }
      acceptedGitCollectionDescriptor = structuredClone(
        gitCollectionDescriptor,
      );
      ...
      for (const [ref, digest, byteLength] of [
        ...
        if (
          ...
          const parsedSummary =
            await parseIndependentReviewTapSummary(transcriptBytes);
          if (
            !(await tapSummaryMatchesExpectation(
              parsedSummary,
              command.testExpectation,
            )) ||
            canonicalizeProjectJson(parsedSummary) !==
              canonicalizeProjectJson(attestation.testSummary)
          ) {
            return fail();
          }
        }
...
async function bundleBaseReasonCodes(bundle, policy) {
  ...
  const gitDiffCheckSubject =
    gitDiffCheck && typeof gitDiffCheck === "object"
      ...
      : null;
  const usesRequiredCheckDiffPathspecs = () =>
    gitDiffCheck?.runnerGitBlobSha256 ===
      "sha256:b70555209f4a6bf833bf0f396ceda09fa3db188d75b5b93dc7d3abaa032242af" &&
    gitDiffCheck?.baseCommit ===
  ...
  if (
    ...
    gitDiffCheck?.logicalCommandSha256 !==
      (await sha256ProjectValue({
        ...
        terminator: "--",
        ...(usesRequiredCheckDiffPathspecs() && { pathspecs: [
            ":(top,glob)**",
...
export async function buildEnvelope({
  contract,reviewBinding,lineage,testWhitelist,commands,inputs,evidence,
  subjects,fullFiles,patches,
}) {
  const withOrigin=(item,originCommit)=>({...item,originCommit});
  const inventoryItems=[...subjects,...inputs.map(item=>withOrigin(item,reviewBinding.implementationCommit)),
    ...evidence.map(item=>withOrigin(item,reviewBinding.finalHead))].sort((left,right)=>left.path<right.path?-1:1);
  const inventoryContract=contract.inventoryLayout;
  const {originLabels,...inventoryLayout}=inventoryContract.subjectInventory;
  const originCommitByLabel={M2A:reviewBinding.implementationCommit,M2B:reviewBinding.finalHead};
  let originLineageIndexes;
  const resolveOriginLineageIndexes=()=>originLineageIndexes??=originLabels.map(label=>{
    const originCommit=originCommitByLabel[label]??label;
    return lineage.findIndex(item=>item.commit===originCommit);
  });
  const inventoryPaths=inventoryLayout.relativePaths.map((path,index)=>
    inventoryLayout.pathPrefixes[inventoryLayout.pathPrefixIndexes[index]]+path);
  const parseSubjectJson=item=>parseIndependentReviewJsonBytes(item.bytes,item.path);
  const jsonValueAt=(value,[key,...rest])=>{
    if(key===undefined)return value;
    if(key==="*")return value.map(item=>jsonValueAt(item,rest));
    return jsonValueAt(value[key],rest);
  };
  const canonical=canonicalizeProjectJson,hashValue=sha256ProjectValue;
  const inventoryMatches=()=>inventoryPaths.every((path,index)=>path===inventoryItems[index]?.path&&
    lineage[resolveOriginLineageIndexes()[inventoryLayout.originCommitIndexes[index]]]?.commit===
      inventoryItems[index].originCommit);
  const lineageClosed=()=>lineage.every((item,index)=>!index||item.parent===lineage[index-1].commit);
  if(!inventoryMatches())contractError();
  if(new Set(inventoryPaths).size!==101)contractError();
  if(!lineageClosed())contractError();
  for(const [index,jsonPath,expected] of contract.semanticChecks){
    const item=index<0?fullFiles[-index-1]:inventoryItems[index];
    const text=item.bytes.toString().toLowerCase();
    let failed;
    if(jsonPath===null)failed=expected.some(value=>text.includes(value));
    else if(jsonPath===true)failed=expected.some(value=>!text.includes(value));
    else failed=canonical(jsonValueAt(parseSubjectJson(item),jsonPath))!==canonical(expected);
    if(failed)contractError();
  }
  await Promise.all(contract.derived.map(async([path,field,expected])=>{
    const item=subjects.find(subject=>subject.path===path),value=parseSubjectJson(item),embedded=value[field];
    if(field)delete value[field];
    item.derivedField=field;
    item.derivedSha256=await hashValue(value);
    if((field?embedded:expected)!==item.derivedSha256)contractError();
  }));
  const fullObjectHashes=[],deletedFieldHashes={subjects:[],fields:[],sha256Hex:[]};
  inventoryItems.forEach((item,index)=>{
    if(item.derivedSha256&&item.derivedField===null){
      fullObjectHashes.push({subjectIndex:index,derivedSha256Hex:item.derivedSha256.slice(7)});
    }else if(item.derivedSha256){
      deletedFieldHashes.subjects.push(index);
      deletedFieldHashes.fields.push(item.derivedField);
      deletedFieldHashes.sha256Hex.push(item.derivedSha256.slice(7));
    }
  });
  const subjectInventory={...inventoryLayout,originLineageIndexes:resolveOriginLineageIndexes(),
    byteLengths:inventoryItems.map(item=>item.byteLength),
    rawSha256Hex:inventoryItems.map(item=>item.rawSha256.slice(7)),
    fullObjectProjectCanonical:fullObjectHashes,deleteFieldProjectCanonical:deletedFieldHashes,
    pathSetSha256:await hashBytes(Buffer.from(`${inventoryPaths.join("\n")}\n`))};
  const lineageSummary={rootParent:lineage[0].parent,sha256Prefix:"sha256:",stages:lineage.map(item=>item.stage),
    commits:lineage.map(item=>item.commit),trees:lineage.map(item=>item.tree),
    pathSetSha256Hex:lineage.map(item=>item.pathSetSha256.slice(7)),
    patchSha256Hex:lineage.map(item=>item.patchSha256.slice(7))};
  const collector={visibility:contract.visibility.collector,testPlanSelfSha256:contract.testPlanSelfSha256,
    staticInputSubjectIndexes:inventoryContract.staticInputSubjectIndexes,
    evidenceFileSubjectIndexes:inventoryContract.evidenceFileSubjectIndexes,commands};
  const coverage={visibility:contract.visibility.digest,testWhitelist,
    formalCollector:collector,subjectInventory};
  const envelope={schemaVersion:contract.envelopeVersion,canonicalization:contract.canonicalization,reviewBinding,
    scopeBoundary:contract.scopeBoundary,lineage:lineageSummary,verbatimCoverage:{visibility:contract.visibility.full},
    digestOnlyCoverage:coverage,semanticSummary:{claims:contract.claims.map(([claimId,value,evidenceRefs])=>
      ({claimId,value,evidenceRefs}))},mechanicalChecks:{passed:true,checks:contract.mechanicalChecks}};
  const patchDescriptors=patches.map(({path,baseCommit,headCommit,byteLength,rawSha256})=>
    ({path,baseCommit,headCommit,byteLength,rawSha256}));
  envelope.verbatimCoverage={...envelope.verbatimCoverage,
    fullFilePaths:fullFiles.map(item=>item.path),patches:patchDescriptors,
    fullFileManifestSha256:await hashValue(fullFiles.map(({path,gitMode,byteLength,rawSha256})=>
      ({path,gitMode,byteLength,rawSha256}))),patchManifestSha256:await hashValue(patchDescriptors)};
  envelope.digestOnlyCoverage={...coverage,manifestSha256:await hashValue(coverage)};
  envelope.envelopeSha256=await hashValue(envelope);
  return{envelope,envelopeBytes:Buffer.from(canonical(envelope))};
}

function contractError() {
  throw new TypeError("INDEPENDENT_MODEL_REVIEW_ENVELOPE_CONTRACT_INVALID");
}
function findingBound(finding, contract) {
  const {path,evidenceMode,evidenceDigest,lineSide,startLine,endLine}=finding;
  const boundSubject=contract.findingSubjects.find(subject=>subject.path===path&&
    subject.evidenceMode===evidenceMode&&subject.evidenceDigest===evidenceDigest);
  if(evidenceMode==="DIGEST_ONLY_SUMMARY")return Boolean(boundSubject)&&
    lineSide===null&&startLine===null&&endLine===null;
  if(!boundSubject||startLine>endLine)return false;
  if(evidenceMode==="VERBATIM_FULL_FILE")return lineSide==="FILE"&&
    startLine>=1&&endLine<=boundSubject.lineCount;
  if(!["OLD","NEW"].includes(lineSide))return false;
  const visibleRanges=lineSide==="OLD"?boundSubject.oldRanges:boundSubject.newRanges;
  return visibleRanges?.some(([firstLine,lastLine])=>
    startLine>=firstLine&&endLine<=lastLine);
}
function outputMatchesRequiredCheck(output, contract) {
  const canonical=canonicalizeProjectJson,findings=output.findings;
  const overclaimGuard=new RegExp(contract.overclaimPattern,"iu");
  const printableAscii=/^[\x20-\x7e]+$/u;
  const summaryMatches=(output.decision==="CLEAR"?output.reviewSummary===contract.reviewSummaryPrefix:
    output.reviewSummary.startsWith(contract.reviewSummaryPrefix))&&printableAscii.test(output.reviewSummary);
  if(!summaryMatches)return false;
  const bindingsMatch=canonical(output.reviewBinding)===canonical(contract.reviewBinding)&&
    canonical(output.coverage)===canonical(contract.coverage);
  if(!bindingsMatch)return false;
  const findingsValid=unique(findings.map(({findingId})=>findingId))&&
    findings.every(finding=>findingBound(finding,contract)&&printableAscii.test(finding.summary));
  if(!findingsValid)return false;
  const clearIsSafe=output.decision!=="CLEAR"||
    findings.every(item=>!overclaimGuard.test(item.evidenceMode+item.findingId+item.summary));
  return clearIsSafe;
}
...
export async function validateIndependentModelReviewOutputArtifact({
  ...
  expectedRequiredCheckContract=null,
}) {
  let output = null;
  try {
  ...
  const valid=expectedRequiredCheckContract===null?outputIsClosed(output):schemaValidation.ok&&
    outputMatchesRequiredCheck(output,expectedRequiredCheckContract);
  if(!valid)reasonCodes.push("INDEPENDENT_REVIEW_OUTPUT_INVALID");
