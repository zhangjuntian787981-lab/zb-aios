...
const TARGETED_REMEDIATION_BINDINGS = Object.freeze({
  evidenceBasisSha256:
    "sha256:09a1f8d9f24547da1df9d45c177f5157e3135f4b46d33d718e9a3a4e5ff05256",
  captureStatusSha256:
    "sha256:f4c1c94f6798980c34d8823fcbd1e21808ae1998a02b9fabbb25c8aeeae25d4e",
  remediationCommitSetSha256:
    "sha256:656bdf8462b87b928dcafc5d2c4719061fee289e9731283b30630a6217a33804",
  reviewRoundSha256: [
    "sha256:16da4dd1b4fc2fefb71092a81b76eb7ca167fdea88f03d688bf50e131004313c",
    "sha256:0021f7448eb959befa77032c5fc07fdddca609aaea5b952a8c308ee30c727f8d",
  ],
  findingSetSha256: [
    "sha256:92edefff6d7c57a3187aa9e0c0c6d432ed7582950ea1765241d2c3d368bc6813",
    "sha256:1c7937191d49a206660b60ff0f80268e652c871c44b638138aaf8d1dbbb7b5fb",
  ],
  engineeringGateSummarySha256:
    "sha256:584a0410a7afefd53f84e8f1506c2e6d47c51e6f16a36bedb5d2b3317a4855ce",
  finalAssessmentSha256:
    "sha256:5cd935e3370504703fa5d9b2453fabed4a88db7128fa823b75f9d9f5ef36fadc",
});
...
const targetedRemediationEvidenceDigest = (evidence) =>
  sha256ProjectValue(withoutField(evidence, "evidenceSha256"));
...
export async function validateTargetedRemediationModelReviewEvidence(
  evidence,
) {
  const reasonCodes = [];
  if (
    !keysExactly(evidence, [
      ...
      "remediationCommitSetSha256",
      "reviewRounds",
      "engineeringGateSummary",
      "finalAssessment",
      "canonicalization",
      "evidenceSha256",
    ]) ||
    evidence?.schemaVersion !==
      "targeted-remediation-model-review-evidence.v1" ||
    evidence?.evidenceType !==
      "TARGETED_REMEDIATION_MODEL_REVIEW_EVIDENCE" ||
    evidence?.evidenceId !== "terra-targeted-remediation-ec8315c" ||
    evidence?.receiptSchemaVersion !==
      "NOT_APPLICABLE_NON_RECEIPT_EVIDENCE" ||
    evidence?.formalReceiptIssued !== false ||
    evidence?.governanceEffect !== "NONE" ||
    evidence?.canonicalization !== "PROJECT_CANONICAL_JSON_V1_NOT_RFC8785"
  ) {
    reasonCodes.push("TARGETED_REMEDIATION_EVIDENCE_INVALID");
  }

  if (
    (await sha256ProjectValue(evidence?.evidenceBasis)) !==
      TARGETED_REMEDIATION_BINDINGS.evidenceBasisSha256 ||
    (await sha256ProjectValue(evidence?.captureStatus)) !==
      TARGETED_REMEDIATION_BINDINGS.captureStatusSha256
  ) {
    reasonCodes.push("TARGETED_REMEDIATION_CAPTURE_BOUNDARY_INVALID");
  }

  const remediationCommitSetSha256 = await sha256ProjectValue(
    evidence?.remediationCommits,
  );
  if (
    evidence?.remediationCommitSetSha256 !==
      TARGETED_REMEDIATION_BINDINGS.remediationCommitSetSha256 ||
    remediationCommitSetSha256 !== evidence?.remediationCommitSetSha256
  ) {
    reasonCodes.push("TARGETED_REMEDIATION_COMMIT_BINDING_INVALID");
  }

  if (!Array.isArray(evidence?.reviewRounds) || evidence.reviewRounds.length !== 2) {
    reasonCodes.push("TARGETED_REMEDIATION_REVIEW_ROUNDS_INVALID");
  } else {
    for (const [index, round] of evidence.reviewRounds.entries()) {
      if (
        (await sha256ProjectValue(round)) !==
          TARGETED_REMEDIATION_BINDINGS.reviewRoundSha256[index] ||
        round?.findingSetSha256 !==
          TARGETED_REMEDIATION_BINDINGS.findingSetSha256[index] ||
        (await sha256ProjectValue(round?.findings)) !== round?.findingSetSha256
      ) {
        reasonCodes.push("TARGETED_REMEDIATION_REVIEW_ROUNDS_INVALID");
        break;
      }
  ...
  if (
    (await sha256ProjectValue(evidence?.engineeringGateSummary)) !==
      TARGETED_REMEDIATION_BINDINGS.engineeringGateSummarySha256
  ) {
    reasonCodes.push("TARGETED_REMEDIATION_GATE_EVIDENCE_INVALID");
  }
  if (
    (await sha256ProjectValue(evidence?.finalAssessment)) !==
    TARGETED_REMEDIATION_BINDINGS.finalAssessmentSha256
  ) {
    reasonCodes.push("TARGETED_REMEDIATION_CLAIM_BOUNDARY_INVALID");
  }
  if (
    !SHA256.test(evidence?.evidenceSha256 ?? "") ||
    (SHA256.test(evidence?.evidenceSha256 ?? "") &&
      (await targetedRemediationEvidenceDigest(evidence)) !==
        evidence.evidenceSha256)
  ) {
    reasonCodes.push("TARGETED_REMEDIATION_EVIDENCE_HASH_MISMATCH");
  }
  return reasonCodes.length === 0
    ? result(true, "VALID_TARGETED_REMEDIATION_EVIDENCE", [])
