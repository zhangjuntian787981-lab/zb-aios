...
export async function buildEnvelope({contract,reviewBinding:binding,lineage,testWhitelist,commands,inputs,evidence,
  subjects,fullFiles,patches}) {
  const tag=(item,originCommit)=>({...item,originCommit});
  const items=[...subjects,...inputs.map(item=>tag(item,binding.implementationCommit)),
    ...evidence.map(item=>tag(item,binding.finalHead))].sort((left,right)=>left.path<right.path?-1:1);
  const layout=contract.inventoryLayout,{originLabels,...shape}=layout.subjectInventory;
  const origins=originLabels.map(label=>label==="M1A"?binding.implementationCommit:label==="M1B"?binding.finalHead:label);
  const paths=shape.relativePaths.map((path,index)=>shape.pathPrefixes[shape.pathPrefixIndexes[index]]+path);
  const json=item=>parseIndependentReviewJsonBytes(item.bytes,item.path);
  const pick=(value,[key,...rest])=>key===undefined?value:key==="*"?value.map(item=>pick(item,rest)):pick(value[key],rest);
  const canon=canonicalizeProjectJson,hash=sha256ProjectValue;
  const inventoryMatches=paths.every((path,index)=>path===items[index]?.path&&
    origins[shape.originCommitIndexes[index]]===items[index].originCommit);
  const lineageClosed=lineage.every((item,index)=>!index||item.parent===lineage[index-1].commit);
  if(!inventoryMatches||new Set(paths).size!==92||!lineageClosed)contractError();
  for(const [index,jsonPath,want] of contract.semanticChecks){
    const item=index<0?fullFiles[-index-1]:items[index],text=item.bytes.toString().toLowerCase();
    const failed=jsonPath===null?want.some(value=>text.includes(value)):jsonPath===true?
      want.some(value=>!text.includes(value)):canon(pick(json(item),jsonPath))!==canon(want);
    if(failed)contractError();
  }
  await Promise.all(contract.derived.map(async([path,field,want])=>{
    const item=subjects.find(subject=>subject.path===path),value=json(item),embedded=value[field];
    if(field)delete value[field];item.derivedField=field;item.derivedSha256=await hash(value);
    if((field?embedded:want)!==item.derivedSha256)contractError();
  }));
  const wholeDigests=[],deletedIndexes=[],deletedFields=[],deletedDigests=[];
  items.forEach((item,index)=>{if(item.derivedSha256&&item.derivedField===null)
    wholeDigests.push({subjectIndex:index,derivedSha256Hex:item.derivedSha256.slice(7)});else if(item.derivedSha256){
    deletedIndexes.push(index);deletedFields.push(item.derivedField);deletedDigests.push(item.derivedSha256.slice(7));}});
  const inventory={...shape,originCommits:origins,byteLengths:items.map(item=>item.byteLength),
    rawSha256Hex:items.map(item=>item.rawSha256.slice(7)),fullObjectProjectCanonical:wholeDigests,
    deleteFieldProjectCanonical:{subjects:deletedIndexes,fields:deletedFields,sha256Hex:deletedDigests},
    pathSetSha256:await hashBytes(Buffer.from(`${paths.join("\n")}\n`))};
  const lineageSummary={rootParent:lineage[0].parent,sha256Prefix:"sha256:",stages:lineage.map(item=>item.stage),
    ...
    pathSetSha256Hex:lineage.map(item=>item.pathSetSha256.slice(7)),patchSha256Hex:lineage.map(item=>item.patchSha256.slice(7))};
  const collector={visibility:contract.visibility.collector,implementationCommit:binding.implementationCommit,
    implementationTree:binding.implementationTree,testPlanSelfSha256:contract.testPlanSelfSha256,
    staticInputSubjectIndexes:layout.staticInputSubjectIndexes,evidenceFileSubjectIndexes:layout.evidenceFileSubjectIndexes,commands};
  const digest={visibility:contract.visibility.digest,testWhitelist,formalCollector:collector,subjectInventory:inventory};
  const envelope={schemaVersion:contract.envelopeVersion,canonicalization:contract.canonicalization,reviewBinding:binding,
    scopeBoundary:contract.scopeBoundary,lineage:lineageSummary,verbatimCoverage:{visibility:contract.visibility.full},
    digestOnlyCoverage:digest,semanticSummary:{claims:contract.claims.map(([claimId,value,evidenceRefs])=>
      ({claimId,value,evidenceRefs}))},mechanicalChecks:{checks:contract.mechanicalChecks.map(checkId=>({checkId,passed:true}))}};
  const patchList=patches.map(({path,baseCommit,headCommit,byteLength,rawSha256})=>
    ({path,baseCommit,headCommit,byteLength,rawSha256}));
  envelope.verbatimCoverage={...envelope.verbatimCoverage,fullFilePaths:fullFiles.map(item=>item.path),patches:patchList,
    fullFileManifestSha256:await hash(fullFiles.map(({path,gitMode,byteLength,rawSha256})=>({path,gitMode,byteLength,rawSha256}))),
    patchManifestSha256:await hash(patchList)};
  envelope.digestOnlyCoverage={...digest,manifestSha256:await hash(digest)};
  envelope.envelopeSha256=await hash(envelope);
  return{envelope,envelopeBytes:Buffer.from(canon(envelope))};
}

function contractError(){throw new TypeError("INDEPENDENT_MODEL_REVIEW_ENVELOPE_CONTRACT_INVALID")}
function findingBound(finding,rule){
  const {path,evidenceMode:mode,evidenceDigest:digest,startLine:start,endLine:end,lineSide:side}=finding,
    bound=rule.findingSubjects.find(item=>item.path===path&&item.evidenceMode===mode&&item.evidenceDigest===digest);
  if(mode==="DIGEST_ONLY_SUMMARY")return bound&&side===null&&start===null&&end===null;
  const spans=side==="OLD"?bound?.oldRanges:bound?.newRanges;
  return bound&&start<=end&&(mode==="VERBATIM_FULL_FILE"?side==="FILE"&&start>=1&&end<=bound.lineCount:
    ["OLD","NEW"].includes(side)&&spans?.some(([lo,hi])=>start>=lo&&end<=hi));
}
function outputMatchesRequiredCheck(out,rule){
  const canon=canonicalizeProjectJson,guard=new RegExp(rule.overclaimPattern,"iu");
  const claims=[out.reviewSummary.slice(rule.reviewSummaryPrefix.length),
    ...out.findings.map(item=>item.evidenceMode+item.findingId+item.summary)];
  const bindingsMatch=canon(out.reviewBinding)===canon(rule.reviewBinding)&&canon(out.coverage)===canon(rule.coverage);
  const findingsValid=unique(out.findings.map(({findingId})=>findingId))&&out.findings.every(finding=>findingBound(finding,rule));
  const clearIsSafe=out.decision!=="CLEAR"||claims.every(value=>!guard.test(value));
  return out.reviewSummary.startsWith(rule.reviewSummaryPrefix)&&bindingsMatch&&findingsValid&&clearIsSafe;
}
