...
export async function parseIndependentReviewTapSummary(stdoutBytes) {
  ...
  const planMatches = lines
    .map((line, index) => {
      const match = /^1\.\.([0-9]+)$/u.exec(line);
      return match === null
        ? null
        : { index, tests: Number(match[1]) };
    })
    .filter((value) => value !== null);
  const footer =
    planMatches.length === 1
      ? lines.slice(planMatches[0].index)
      : [];
  const tapHeaderIndexes = lines
    .map((line, index) => (line === "TAP version 13" ? index : null))
    .filter((value) => value !== null);
  const topLevelResults = lines
    .map((line) => {
      const match = /^(?:ok|not ok) ([0-9]+) - /u.exec(line);
  ...
  if (
    ...
    planMatches.length !== 1 ||
    tapHeaderIndexes[0] >= planMatches[0].index ||
    topLevelResults.length !== planMatches[0].tests ||
    topLevelResults.some((value, index) => value !== index + 1) ||
    footer.length !== 9 ||
    !/^# tests [0-9]+$/u.test(footer[1] ?? "") ||
    !/^# suites [0-9]+$/u.test(footer[2] ?? "") ||
    !/^# pass [0-9]+$/u.test(footer[3] ?? "") ||
    !/^# fail [0-9]+$/u.test(footer[4] ?? "") ||
    !/^# cancelled [0-9]+$/u.test(footer[5] ?? "") ||
    !/^# skipped [0-9]+$/u.test(footer[6] ?? "") ||
    !/^# todo [0-9]+$/u.test(footer[7] ?? "") ||
    !/^# duration_ms [0-9]+(?:\.[0-9]+)?$/u.test(footer[8] ?? "")
  ) {
    return null;
  }
  const counts = {};
  ...
  const skippedTestNames = sortedUtf8Strings(
    text
      .split(/\r?\n/u)
      .map((line) => /^ok [0-9]+ - (.+) # SKIP$/u.exec(line)?.[1] ?? null)
      .filter((value) => value !== null),
  );
  if (
    planMatches[0].tests > counts.tests ||
    counts.tests !==
      counts.pass +
        counts.fail +
        counts.cancelled +
