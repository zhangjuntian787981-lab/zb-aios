...
function normalizeModel(model) {
  ...
  return Object.freeze(clone(model));
}
...
function normalizeTenantPolicy(policy, knownModels) {
  ...
  return Object.freeze(clone(policy));
}
  ...
  const allowed = evaluated
    ...
    .sort(
      (left, right) =>
        ...
        left.inputMicrousdPerMillion +
          left.outputMicrousdPerMillion -
          (right.inputMicrousdPerMillion +
            right.outputMicrousdPerMillion) ||
...
function calculateCost(model, usage) {
  return (
    Math.ceil(
      (usage.inputTokens * model.inputMicrousdPerMillion) / 1_000_000,
    ) +
    Math.ceil(
      (usage.outputTokens * model.outputMicrousdPerMillion) / 1_000_000,
    )
  );
}
...
export function createModelGateway({
  ...
  async function route(serverContext, request) {
    ...
    const maximumModel = filtered.allowed.reduce((left, right) =>
      left.inputMicrousdPerMillion +
        left.outputMicrousdPerMillion >=
      right.inputMicrousdPerMillion +
        right.outputMicrousdPerMillion
    ...
    const reservedCostMicrousd =
      Math.ceil(
        (dataPolicy.inputTokens *
          maximumModel.inputMicrousdPerMillion) /
          1_000_000,
      ) +
      Math.ceil(
        (task.maxOutputTokens *
          maximumModel.outputMicrousdPerMillion) /
          1_000_000,
      );
      ...
      try {
        routeRecord = await store.completeRoute(
          ...
          Object.freeze({
            ...
            usage,
            rateVersion: model.rateVersion,
            costMicrousd: calculateCost(model, usage),
