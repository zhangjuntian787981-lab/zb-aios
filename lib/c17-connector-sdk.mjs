...
function validCurrentResultTime(provenance, requestedAsOf) {
  if (provenance.observed_at === null) return false;
  const observedAt = Date.parse(
    normalizeC17DateTime(provenance.observed_at),
  );
  const asOf =
    provenance.as_of === null
      ? null
      : Date.parse(normalizeC17DateTime(provenance.as_of));
  const requested =
    requestedAsOf === null ? null : Date.parse(requestedAsOf);
  return (
    (asOf === null || asOf <= observedAt) &&
    (requested === null ||
      ((asOf === null || asOf <= requested) &&
...
function validMockResult(
  ...
  return (
    ...
    validCurrentResultTime(result.provenance, requestedAsOf) &&
...
export function createC17ConnectorSdk(configuration) {
  ...
  const lastAcceptedObservedAtByLookup = new Map();
  ...
  return Object.freeze({
    async execute(serverContext, envelope, capability) {
      ...
      const validatedResult = validateConnectorEnvelope({
      ...
      const orderingKey = c17ConnectorSha256({
        tenantId: serverContext.tenantId,
        operationId: request.operation_id,
        parameters: request.request.parameters,
      });
      const observedAt =
        validatedResult.result.provenance.observed_at;
      const previousObservedAt =
        lastAcceptedObservedAtByLookup.get(orderingKey);
      if (
        previousObservedAt !== undefined &&
        Date.parse(observedAt) < Date.parse(previousObservedAt)
      ) {
        fail(
          "INVALID_ADAPTER_RESULT",
          "C17 Adapter result is invalid.",
      ...
      lastAcceptedObservedAtByLookup.set(orderingKey, observedAt);
