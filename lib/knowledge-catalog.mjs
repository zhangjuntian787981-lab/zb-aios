...
const KNOWLEDGE_NODE_ID = /^knn_[0-9a-f]{32}$/;
...
export function validateKnowledgeProvenance(
  ...
  for (const node of nodes) {
    ...
    if (
      ...
      typeof node !== "object" ||
      Array.isArray(node) ||
      Object.keys(node).length !== fields.length ||
      fields.some((field) => !Object.hasOwn(node, field)) ||
      !KNOWLEDGE_NODE_ID.test(node.nodeId ?? "") ||
      ...
      node.ordinal < 0 ||
      ordinals.has(node.ordinal) ||
      !(
        node.parentNodeId === null ||
        KNOWLEDGE_NODE_ID.test(node.parentNodeId ?? "")
      ) ||
    ...
    validateKnowledgeLocation(node);
  ...
  for (const node of nodes) {
    ...
    if (
      node.location.page !== parent.location.page &&
      parent.nodeType !== "ORIGINAL"
    ) {
      fail("INVALID_PROVENANCE", "Source node page is invalid.");
    }
    if (
      node.location.chunkKind === "TABLE_ROW" &&
      (parent.nodeType !== "TABLE" ||
        node.location.tableNodeId !== parent.nodeId)
    ) {
      fail("INVALID_PROVENANCE", "Table-row location is invalid.");
    }
    if (
      node.nodeType === "CHUNK" &&
      parent.nodeType === "TABLE" &&
      ...
      fail("INVALID_PROVENANCE", "Table Chunk location is invalid.");
    }
...
function validateKnowledgeLocation(node) {
  const location = node.location;
  const keys = Object.keys(location);
  const exact = (expected) =>
    keys.length === expected.length &&
    expected.every((field) => Object.hasOwn(location, field));
  const positive = (field, { allowZero = false } = {}) =>
    Number.isSafeInteger(location[field]) &&
    location[field] >= (allowZero ? 0 : 1);
  let valid = false;
  if (node.nodeType === "ORIGINAL") {
    valid =
      exact(["byteStart", "byteEnd"]) &&
      positive("byteStart", { allowZero: true }) &&
      positive("byteEnd") &&
      location.byteStart === 0 &&
      location.byteStart < location.byteEnd;
  } else if (node.nodeType === "PAGE") {
    valid = exact(["page"]) && positive("page");
  } else if (node.nodeType === "SECTION") {
    valid =
      exact(["page", "lineStart", "headingLevel"]) &&
      positive("page") &&
      positive("lineStart") &&
      positive("headingLevel") &&
      location.headingLevel <= 6;
  } else if (node.nodeType === "TABLE") {
    const lineTable =
      exact(["page", "lineStart", "lineEnd"]) &&
      positive("lineStart") &&
      positive("lineEnd") &&
      location.lineStart <= location.lineEnd;
    const rowTable =
      exact(["page", "rowStart", "rowEnd"]) &&
      positive("rowStart") &&
      positive("rowEnd") &&
      location.rowStart <= location.rowEnd;
    valid = positive("page") && (lineTable || rowTable);
  } else if (node.nodeType === "CHUNK") {
    const paragraph =
      location.chunkKind === "PARAGRAPH" &&
      exact(["page", "lineStart", "lineEnd", "chunkKind"]) &&
      positive("lineStart") &&
      positive("lineEnd") &&
      location.lineStart <= location.lineEnd;
    const tableRow =
      location.chunkKind === "TABLE_ROW" &&
      exact(["page", "tableNodeId", "row", "chunkKind"]) &&
      KNOWLEDGE_NODE_ID.test(location.tableNodeId ?? "") &&
      positive("row");
    valid = positive("page") && (paragraph || tableRow);
  }
  if (!valid) {
    fail("INVALID_PROVENANCE", "Source node location is invalid.");
  }
}

function closedParserNodes(nodes, expectedContentSha256) {
  validateKnowledgeProvenance(nodes, expectedContentSha256);
  return Object.freeze(
    nodes.map((node) =>
      Object.freeze({
        nodeId: node.nodeId,
        ...
        location: Object.freeze({ ...node.location }),
...
function validateParserResult(result, input) {
  ...
  const nodes = closedParserNodes(result.nodes, input.contentSha256);
  const original = nodes.find((node) => node.nodeType === "ORIGINAL");
  if (original.location.byteEnd !== input.content.byteLength) {
    fail(
      "INVALID_PARSER_RESULT",
      "Parser original byte range is inconsistent.",
    );
  }
  const counts = {
    pageCount: nodes.filter((node) => node.nodeType === "PAGE").length,
    sectionCount: nodes.filter(
  ...
  if (
    ...
    result.parseSha256 !== c10Sha256(nodes)
  ) {
  ...
  return Object.freeze({
    ...
    parserRoute: result.parserRoute,
    verificationScope: result.verificationScope,
    productionOcrVerified: result.productionOcrVerified,
    parserVersion: result.parserVersion,
    authorityStatus: result.authorityStatus,
    ...counts,
    nodes,
    parseSha256: result.parseSha256,
  });
}
...
export function transitionKnowledgeDocument({
  ...
  if (command.type === "PREPARE_UPLOAD") {
  ...
  if (command.type === "COMPLETE_UPLOAD") {
    if (
      current.state !== "UPLOAD_PENDING" ||
      current.fixtureRef !== command.fixtureRef ||
      current.sourceRef !== command.sourceRef ||
      current.filename !== command.filename ||
      current.declaredMediaType !== command.declaredMediaType ||
      current.contentSha256 !== command.contentSha256 ||
      current.contentSize !== command.contentSize ||
      current.quarantineRef !== command.quarantineRef
    ) {
      fail("INVALID_STATE", "Pending upload does not match its effect.");
    }
    return Object.freeze({
      ...clone(current),
      state: "QUARANTINED",
  ...
  if (command.type === "PREPARE_DELETE") {
    if (
      ["UPLOAD_PENDING", "DELETE_PENDING", "DELETED"].includes(
        current.state,
      )
    ) {
      fail(
        "INVALID_STATE",
        "Document is not deletable while storage is pending or final.",
      );
    }
    return Object.freeze({
      ...clone(current),
      state: "DELETE_PENDING",
      nodes: withNodeAvailability(
        current.nodes,
        "DELETE_PENDING",
        command.at,
      ),
  ...
  if (command.type === "COMPLETE_DELETE") {
    if (current.state !== "DELETE_PENDING") {
      fail("INVALID_STATE", "Document deletion is not pending.");
    }
    return Object.freeze({
      ...clone(current),
      revision: current.revision + 1,
      state: "DELETED",
      nodes: withNodeAvailability(current.nodes, "DELETED", command.at),
      authorizationEvidence: clone(command.authorizationEvidence),
      updatedAt: command.at,
...
export function createMemoryKnowledgeCatalogStore({ snapshot } = {}) {
  ...
  const storageEffects = new Map(snapshot?.storageEffects ?? []);
  ...
  function receipt(scope, request) {
    const storedReceipt = receipts.get(
      receiptKey(scope.tenantId, request.idempotencyKey),
    );
    if (!storedReceipt) return null;
    if (storedReceipt.requestHash !== request.requestHash) {
      fail(
  ...
  function effect(scope, request) {
    const storedEffect = storageEffects.get(
      receiptKey(scope.tenantId, request.idempotencyKey),
    );
    if (!storedEffect) return null;
    if (
      storedEffect.requestHash !== request.requestHash ||
      storedEffect.effectKind !== request.effectKind
    ) {
      fail(
        "IDEMPOTENCY_CONFLICT",
        "Storage-effect key was reused with another request.",
      );
    }
    return storedEffect;
  }
  ...
  return Object.freeze({
    async readReceipt(scope, request) {
      validateTenantScope(scope);
      return receipt(scope, request);
    },
    async apply(scope, envelope) {
      validateTenantScope(scope);
      return serialized(async () => {
        const replay = receipt(scope, envelope);
    ...
    async prepareStorageEffect(scope, envelope) {
      validateTenantScope(scope);
      if (
        !["PUT", "ERASE"].includes(envelope.effectKind) ||
        (envelope.effectKind === "PUT" &&
          envelope.command?.type !== "UPLOAD") ||
        (envelope.effectKind === "ERASE" &&
          envelope.command?.type !== "DELETE")
      ) {
        fail("INVALID_INPUT", "Storage effect is invalid.");
      }
      return serialized(async () => {
        const replay = receipt(scope, envelope);
        if (replay) return replay;
        const storedEffect = effect(scope, envelope);
        if (storedEffect) {
          if (storedEffect.status !== "PENDING") {
            fail(
              "INTEGRITY_VIOLATION",
              "Completed storage effect has no command receipt.",
            );
          }
          const current = documents.get(
            documentKey(
              scope.tenantId,
              storedEffect.documentId,
              storedEffect.documentVersion,
            ),
          );
          return {
            pending: true,
            effect: clone(storedEffect),
        ...
        documents.set(key, clone(next));
        const pendingEffect = Object.freeze({
          tenantId: scope.tenantId,
          tenantKind: "SYNTHETIC",
          idempotencyKey: envelope.idempotencyKey,
          requestHash: envelope.requestHash,
          effectKind: envelope.effectKind,
          status: "PENDING",
          documentId: next.documentId,
          documentVersion: next.documentVersion,
          quarantineRef: next.quarantineRef,
          contentSha256: next.contentSha256,
          command: clone(envelope.command),
          createdAt: envelope.command.at,
          completedAt: null,
        });
        storageEffects.set(
        ...
        return {
          pending: true,
          effect: clone(pendingEffect),
          document: clone(next),
        };
      });
    },
    async completeStorageEffect(scope, envelope) {
      validateTenantScope(scope);
      return serialized(async () => {
        const replay = receipt(scope, envelope);
        if (replay) return replay;
        const storedEffect = effect(scope, envelope);
        if (!storedEffect || storedEffect.status !== "PENDING") {
          fail("NOT_FOUND", "Pending storage effect does not exist.");
        }
        const key = documentKey(
          scope.tenantId,
          storedEffect.documentId,
        ...
        storageEffects.set(
          receiptKey(scope.tenantId, envelope.idempotencyKey),
          {
            ...clone(storedEffect),
            status: "COMPLETED",
            completedAt: envelope.command.at,
          },
        );
        return result;
      });
    },
    async listPendingStorageEffects(scope, { limit = 100 } = {}) {
      validateTenantScope(scope);
      safePositiveInteger(limit, "limit");
      if (limit > 1000) fail("INVALID_INPUT", "limit is too large.");
      return [...storageEffects.values()]
        .filter(
          (entry) =>
    ...
    async readAsOf(scope, { documentId, documentVersion, asOf }) {
      ...
      for (const version of versions) {
        ...
        const live = documents.get(
          documentKey(scope.tenantId, documentId, version),
        );
        if (
          live?.state === "DELETE_PENDING" &&
          Date.parse(live.updatedAt) <= Date.parse(asOf)
        ) {
          continue;
        }
        const history =
    ...
    exportSnapshot() {
      return clone({
        ...
        storageEffects: [...storageEffects.entries()],
      });
    },
  });
}
...
function externalRequestHash(type, tenantId, command) {
  const stableRequest = { ...command };
  delete stableRequest.idempotencyKey;
  return c10Sha256({
    type,
    tenantId,
    ...stableRequest,
...
function externalEnvelope(command, idempotencyKey, requestHash) {
  return Object.freeze({ idempotencyKey, requestHash, command });
}
...
export function createKnowledgeCatalog({
  ...
  if (
    ...
    typeof store?.readReceipt !== "function" ||
    typeof store?.prepareStorageEffect !== "function" ||
    typeof store?.completeStorageEffect !== "function" ||
    typeof store?.listPendingStorageEffects !== "function" ||
  ...
  async function finishStorageEffect(
    scope,
    pendingEffect,
    ...
    if (pendingEffect.effectKind === "PUT") {
      const { content } = benchmark.resolve(command.fixtureRef);
      if (
        c10Sha256(content) !== pendingEffect.contentSha256 ||
        content.byteLength !== command.contentSize
      ) {
        fail("CONTENT_HASH_MISMATCH", "Pending upload bytes changed.");
      }
      await quarantineStore.put({
        tenantId: scope.tenantId,
        quarantineRef: pendingEffect.quarantineRef,
        contentSha256: pendingEffect.contentSha256,
        content,
      });
    } else if (pendingEffect.effectKind === "ERASE") {
      ...
      fail("INTEGRITY_VIOLATION", "Pending storage effect is invalid.");
    }
    return store.completeStorageEffect(scope, {
      idempotencyKey: pendingEffect.idempotencyKey,
      requestHash: pendingEffect.requestHash,
  ...
  return Object.freeze({
    async upload(context, command) {
      ...
      const requestHash = externalRequestHash(
        "UPLOAD",
      ...
      const replay = await store.readReceipt(context.tenantScope, {
      ...
      const pending = await store.prepareStorageEffect(
        context.tenantScope,
        {
          idempotencyKey: command.idempotencyKey,
          requestHash,
      ...
      if (pending.replayed) return pending;
      return finishStorageEffect(
        context.tenantScope,
        pending.effect,
        authorizationEvidence,
      );
    },
    async reconcileStorageEffects(context, options = {}) {
      exactKeys(
        options,
        ["limit"],
        "C10 reconciliation options",
        [],
      );
      ...
      if (limit > 1000) fail("INVALID_INPUT", "limit is too large.");
      validateContext(context);
      const pendingEffects = await store.listPendingStorageEffects(
        context.tenantScope,
        { limit },
      );
      const results = [];
      for (const pendingEffect of pendingEffects) {
        const authorizationEvidence = await commandContext(
          context,
          pendingEffect.documentId,
          MUTATION_SURFACE,
        );
        results.push(
          await finishStorageEffect(
            context.tenantScope,
            pendingEffect,
            authorizationEvidence,
          ),
