...
const BASE_PACKAGE_VERSIONS = Object.freeze({
  ajv: "8.20.0",
  "ajv-formats": "2.1.1",
...
const BASE_PACKAGE_ENTRYPOINTS = Object.freeze({
  ajv: "dist/2020.js",
...
const K3_V3_PACKAGE_VERSIONS = Object.freeze({
  ...BASE_PACKAGE_VERSIONS,
  undici: "7.24.8",
});
const K3_V3_PACKAGE_ENTRYPOINTS = Object.freeze({
  ...BASE_PACKAGE_ENTRYPOINTS,
  undici: "index.js",
});
...
export const kimiK3IndependentReviewRuntimeLocalModulePaths = Object.freeze([
  "lib/independent-model-review.mjs",
  "lib/independent-review-runtime-binding.mjs",
  "lib/independent-review-runtime-manifest.mjs",
  "lib/independent-review-transport-evidence.mjs",
...
const RUNTIME_MANIFEST_CONTRACTS = Object.freeze({
  "independent-review-runtime-dependency-manifest.v1": Object.freeze({
    schemaVersion: "independent-review-runtime-dependency-manifest.v1",
    localModulePaths: independentReviewRuntimeLocalModulePaths,
    packageVersions: BASE_PACKAGE_VERSIONS,
    packageEntrypoints: BASE_PACKAGE_ENTRYPOINTS,
  }),
  "independent-review-runtime-dependency-manifest.v2": Object.freeze({
    schemaVersion: "independent-review-runtime-dependency-manifest.v2",
    localModulePaths: kimiK3IndependentReviewRuntimeLocalModulePaths,
    packageVersions: BASE_PACKAGE_VERSIONS,
    packageEntrypoints: BASE_PACKAGE_ENTRYPOINTS,
  }),
  "independent-review-runtime-dependency-manifest.v3": Object.freeze({
    schemaVersion: "independent-review-runtime-dependency-manifest.v3",
    localModulePaths: kimiK3IndependentReviewRuntimeLocalModulePaths,
    packageVersions: K3_V3_PACKAGE_VERSIONS,
    packageEntrypoints: K3_V3_PACKAGE_ENTRYPOINTS,
    requiredRootDevDependencies: Object.freeze({ undici: "7.24.8" }),
  }),
  "independent-review-runtime-dependency-manifest.v4": Object.freeze({
    schemaVersion: "independent-review-runtime-dependency-manifest.v4",
    localModulePaths: kimiK3IndependentReviewRuntimeLocalModulePaths,
    packageVersions: K3_V3_PACKAGE_VERSIONS,
    packageEntrypoints: K3_V3_PACKAGE_ENTRYPOINTS,
    requiredRootDevDependencies: Object.freeze({ undici: "7.24.8" }),
  }),
  "independent-review-runtime-dependency-manifest.v5": Object.freeze({
    schemaVersion: "independent-review-runtime-dependency-manifest.v5",
    localModulePaths: kimiK3IndependentReviewRuntimeLocalModulePaths,
    packageVersions: K3_V3_PACKAGE_VERSIONS,
    packageEntrypoints: K3_V3_PACKAGE_ENTRYPOINTS,
    requiredRootDevDependencies: Object.freeze({ undici: "7.24.8" }),
...
async function resolveFrozenDependencyRoot(dependencyRoot, packageNames) {
  const exactInputRoot = await realpath(resolve(dependencyRoot));
  if (!(await stat(exactInputRoot)).isDirectory()) {
    throw new TypeError("Runtime dependency root is invalid.");
  }
  const packageRoots = await Promise.all(
    packageNames.map((name) => realpath(resolve(exactInputRoot, name))),
  );
  const exactContentRoot = dirname(packageRoots[0]);
  if (
    packageRoots.some(
      (packageRoot, index) =>
        dirname(packageRoot) !== exactContentRoot ||
        packageRoot !== resolve(exactContentRoot, packageNames[index]),
    )
  ) {
    throw new TypeError(
      "Runtime dependencies do not share one frozen content root.",
    );
  }
  return exactContentRoot;
}
...
async function captureRuntimeDependencyManifestForContract(
  {
    sourceRoot,
    dependencyRoot,
  ...
  const packageNames = Object.freeze(
    Object.keys(contract.packageVersions).sort(),
  );
  assertIndependentReviewBootstrapEnvironment({
    requiredExecArgv,
  });
  const exactSourceRoot = await realpath(resolve(sourceRoot));
  const exactDependencyRoot =
    await resolveFrozenDependencyRoot(dependencyRoot, packageNames);
  ...
  for (const [name, version] of Object.entries(
    contract.requiredRootDevDependencies ?? {},
  )) {
    if (packageJson?.devDependencies?.[name] !== version) {
      throw new TypeError("Required runtime dependency is not directly pinned.");
    }
  }
  ...
  for (const name of packageNames) {
    ...
    if (
      ...
      packageValue?.version !== contract.packageVersions[name] ||
      lockValue?.version !== contract.packageVersions[name] ||
    ...
    const entryPath = `${name}/${contract.packageEntrypoints[name]}`;
  ...
  const localModuleSubjects = await Promise.all(
    contract.localModulePaths.map((path) =>
...
export async function captureIndependentReviewRuntimeDependencyManifest(
  ...
  return captureRuntimeDependencyManifestForContract(
    input,
    RUNTIME_MANIFEST_CONTRACTS[
      "independent-review-runtime-dependency-manifest.v1"
    ],
  );
}

export async function captureKimiK3IndependentReviewRuntimeDependencyManifest(
  input = {},
) {
  return captureRuntimeDependencyManifestForContract(
    input,
...
export async function captureKimiK3IndependentReviewRuntimeDependencyManifestV3(
  input = {},
) {
  return captureRuntimeDependencyManifestForContract(
    input,
...
export async function captureKimiK3IndependentReviewRuntimeDependencyManifestV4(
  input = {},
) {
  return captureRuntimeDependencyManifestForContract(
    input,
...
export async function captureKimiK3IndependentReviewRuntimeDependencyManifestV5(
  input = {},
) {
  return captureRuntimeDependencyManifestForContract(
    input,
...
export function validateIndependentReviewRuntimeDependencyManifest(
  ...
  const contract = RUNTIME_MANIFEST_CONTRACTS[manifest?.schemaVersion];
  const packageNames = contract
    ? Object.keys(contract.packageVersions).sort()
    : [];
  const valid =
    Boolean(contract) &&
    ...
    manifest.source.localModuleSubjects.length ===
      contract.localModulePaths.length &&
    JSON.stringify(
      manifest.source.localModuleSubjects.map(({ path }) => path),
    ) === JSON.stringify(contract.localModulePaths) &&
    ...
    manifest.dependencies.criticalPackages.every(
      (entry) =>
        ...
        contract.packageVersions[entry.name] === entry.version &&
