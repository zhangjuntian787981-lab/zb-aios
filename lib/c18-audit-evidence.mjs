...
export function createSyntheticAuditEvidenceRegistry(document) {
  ...
  for (const entry of document.entries) {
    ...
    if (entry.evidenceType === "IDENTITY") {
      validateActionIdentityArtifact(entry.artifact, entry.tenantId);
    } else {
      exactKeys(
        entry.artifact,
        [
      ...
      if (
        ...
        !/^[A-Z][0-9]{2}$/.test(
          entry.artifact.sourceWorkPackage ?? "",
        ) ||
  ...
  for (const entry of entries.values()) {
    const key =
      `${entry.tenantId}\u0000${entry.evidenceType}\u0000${entry.sha256}`;
    if (artifactEntries.has(key)) {
      fail(
        "INVALID_EVIDENCE_REGISTRY",
        "Duplicate Synthetic evidence artifact.",
      );
    }
    artifactEntries.set(key, entry);
  }
  ...
  return Object.freeze({
    ...
    resolveArtifact({ tenantId, evidenceType, artifact }) {
      validateTenantId(tenantId);
      if (!EVIDENCE_TYPES.has(evidenceType)) {
        fail("INVALID_EVIDENCE_REGISTRY", "Evidence type is invalid.");
      }
      if (evidenceType === "IDENTITY") {
        validateActionIdentityArtifact(artifact, tenantId);
      }
      const sha256 = auditEvidenceSha256(artifact);
      const entry = artifactEntries.get(
        `${tenantId}\u0000${evidenceType}\u0000${sha256}`,
      );
      if (
        !entry ||
        canonicalizeAuditJson(entry.artifact) !==
          canonicalizeAuditJson(artifact)
      ) {
        fail(
          "EVIDENCE_DIGEST_MISMATCH",
          "Synthetic evidence artifact did not resolve.",
        );
      }
      return clone(entry);
    },
...
export function createSyntheticAuditEvidenceCatalog(
  ...
  if (
    typeof evidenceRegistry?.resolve !== "function" ||
    typeof evidenceRegistry?.resolveArtifact !== "function"
  ) {
    fail(
  ...
  return Object.freeze({
    ...
    resolveIdentityArtifact(tenantId, artifact) {
      return evidenceRegistry.resolveArtifact({
        tenantId,
...
function actionIdentityArtifact(identity) {
  const snapshot = actionIdentitySnapshot(identity);
  return {
    schemaVersion: "c18-action-identity-artifact.v1",
    tenantId: snapshot.tenantId,
    evidenceType: "IDENTITY",
    artifactId: `synthetic-action-identity-${snapshot.tenantId}`,
    sourceWorkPackage: "C05",
    identityAccountSha256: auditEvidenceSha256(
      snapshot.identityAccountId,
    ),
    identityLinkSha256: auditEvidenceSha256(snapshot.identityLinkId),
    sessionSha256: auditEvidenceSha256(snapshot.sessionId),
    purposeRef: snapshot.purposeRef,
    humanSubject: {
      principalRef:
        `evidence://c05/principals/${snapshot.humanSubject.principalId}`,
      lifecycleVersion: snapshot.humanSubject.lifecycleVersion,
      securityEpoch: snapshot.humanSubject.securityEpoch,
    },
    workloadActor: {
      principalRef:
        `evidence://c05/principals/${snapshot.workloadActor.principalId}`,
      principalType: snapshot.workloadActor.principalType,
      lifecycleVersion: snapshot.workloadActor.lifecycleVersion,
      securityEpoch: snapshot.workloadActor.securityEpoch,
    },
    delegationChain: snapshot.delegationChain.map((entry) => ({
      delegationRef:
        `evidence://c05/delegations/${entry.delegationId}`,
      delegatorPrincipalRef:
        `evidence://c05/principals/${entry.delegatorPrincipalId}`,
      delegatePrincipalRef:
        `evidence://c05/principals/${entry.delegatePrincipalId}`,
      purposeRef: entry.purposeRef,
      lifecycleVersion: entry.lifecycleVersion,
      expiresAt: entry.expiresAt,
    })),
    trustSource: snapshot.trustSource,
  };
}

function validateActionIdentityArtifact(value, expectedTenantId) {
  exactKeys(
    value,
    [
      "schemaVersion",
    ...
    "Action identity artifact",
  );
  if (
    value.schemaVersion !== "c18-action-identity-artifact.v1" ||
    value.tenantId !== expectedTenantId ||
    value.evidenceType !== "IDENTITY" ||
    value.artifactId !==
      `synthetic-action-identity-${expectedTenantId}` ||
    value.sourceWorkPackage !== "C05" ||
    value.trustSource !==
      "VERIFIED_SESSION_IDENTITY_LINK_AND_WORKLOAD_CONTEXT"
  ) {
    fail(
      "INVALID_IDENTITY_ARTIFACT",
      "Action identity artifact constants are invalid.",
    );
  }
  ...
  for (const field of [
    ...
    if (!SHA256.test(value[field] ?? "")) {
      fail(
        "INVALID_IDENTITY_ARTIFACT",
        `Action identity ${field} is invalid.`,
      );
    }
  }
  reference(value.purposeRef, "identity artifact purposeRef");
  exactKeys(
    value.humanSubject,
    ["principalRef", "lifecycleVersion", "securityEpoch"],
    "identity artifact humanSubject",
  );
  exactKeys(
    value.workloadActor,
    [
    ...
    "identity artifact workloadActor",
  );
  if (
    !/^evidence:\/\/c05\/principals\/prn_/.test(
      value.humanSubject.principalRef ?? "",
    ) ||
    !/^evidence:\/\/c05\/principals\/prn_/.test(
      value.workloadActor.principalRef ?? "",
    ) ||
    !["AGENT", "SERVICE"].includes(value.workloadActor.principalType)
  ) {
    fail(
      "INVALID_IDENTITY_ARTIFACT",
      "Action identity principals are invalid.",
    );
  }
  for (const [field, item] of [
    ["human lifecycle", value.humanSubject.lifecycleVersion],
    ["human security epoch", value.humanSubject.securityEpoch],
    ["workload lifecycle", value.workloadActor.lifecycleVersion],
    ["workload security epoch", value.workloadActor.securityEpoch],
  ]) {
    positiveInteger(item, field);
  }
  if (
    !Array.isArray(value.delegationChain) ||
    value.delegationChain.length === 0
  ) {
    fail(
      "INVALID_IDENTITY_ARTIFACT",
      "Action identity delegation is invalid.",
    );
  }
  for (const [index, delegation] of value.delegationChain.entries()) {
    exactKeys(
      delegation,
      [
        "delegationRef",
      ...
      `identity artifact delegation[${index}]`,
    );
    for (const [field, item] of [
      ["delegationRef", delegation.delegationRef],
      ["delegatorPrincipalRef", delegation.delegatorPrincipalRef],
      ["delegatePrincipalRef", delegation.delegatePrincipalRef],
      ["purposeRef", delegation.purposeRef],
    ]) {
      reference(item, `identity artifact ${field}`);
    }
    if (
      !/^evidence:\/\/c05\/delegations\/dlg_/.test(
        delegation.delegationRef,
      ) ||
      !/^evidence:\/\/c05\/principals\/prn_/.test(
        delegation.delegatorPrincipalRef,
      ) ||
      !/^evidence:\/\/c05\/principals\/prn_/.test(
        delegation.delegatePrincipalRef,
      )
    ) {
      fail(
        "INVALID_IDENTITY_ARTIFACT",
        "Action identity delegation references are invalid.",
      );
    }
    positiveInteger(
      delegation.lifecycleVersion,
      `identity artifact delegation[${index}].lifecycleVersion`,
    );
    canonicalInstant(
      delegation.expiresAt,
      `identity artifact delegation[${index}].expiresAt`,
    );
    const expectedDelegator =
      index === 0
        ? value.humanSubject.principalRef
        : value.delegationChain[index - 1].delegatePrincipalRef;
    if (delegation.delegatorPrincipalRef !== expectedDelegator) {
      fail(
        "INVALID_IDENTITY_ARTIFACT",
        "Action identity delegation chain is invalid.",
      );
    }
  }
  if (
    value.delegationChain.at(-1).delegatePrincipalRef !==
    value.workloadActor.principalRef
  ) {
    fail(
      "INVALID_IDENTITY_ARTIFACT",
      "Action identity workload binding is invalid.",
    );
  }
...
function identityEvidence(identity, catalog) {
  const artifact = actionIdentityArtifact(identity);
  const registered = catalog.resolveIdentityArtifact(
    identity.tenantId,
    artifact,
  );
  const leaf = artifact.delegationChain.at(-1);
  return {
    evidenceRef: registered.evidenceRef,
    version: registered.version,
    sha256: registered.sha256,
    artifact,
    humanPrincipalRef: artifact.humanSubject.principalRef,
    workloadPrincipalRef: artifact.workloadActor.principalRef,
    delegationRef: leaf.delegationRef,
  };
}
...
function payloadFor({
  ...
  const identityItem = identityEvidence(identity, catalog);
...
function validateIdentityEvidence(value, expectedTenantId) {
  exactKeys(
    value,
    [
      "evidenceRef",
  ...
  if (
    evidence.version !== "c18-action-identity-artifact-v1" ||
    evidence.evidenceRef !==
      `evidence://c05/action-identities/${expectedTenantId}`
  ) {
    fail("INVALID_AUDIT_PAYLOAD", "Identity evidence is not registered.");
  }
  const artifact = validateActionIdentityArtifact(
    value.artifact,
    expectedTenantId,
  );
  const leaf = artifact.delegationChain.at(-1);
  if (
    auditEvidenceSha256(artifact) !== evidence.sha256 ||
    value.humanPrincipalRef !== artifact.humanSubject.principalRef ||
    value.workloadPrincipalRef !== artifact.workloadActor.principalRef ||
    value.delegationRef !== leaf.delegationRef
  ) {
    fail(
      "INVALID_AUDIT_PAYLOAD",
      "Identity evidence artifact digest is invalid.",
    );
  }
  for (const [field, item] of [
...
function validateAuditPayload(value, expected) {
  ...
  const identity = validateIdentityEvidence(
    value.identity,
    expected.tenantId,
  );
...
function headView(tenantId, events) {
  ...
  return {
    ...
    lastEventHash: last?.eventHash ?? GENESIS_HASH,
    updatedAt: last?.createdAt ?? null,
  };
}

function deliveryIntentView(entry) {
  return Object.freeze({
    eventId: entry.eventId,
    event: clone(entry.event),
...
function recoveryDocument(tenantId, state) {
  const body = recoveryBody({
    tenantId,
    head: headView(tenantId, state.events),
    events: state.events,
    deliveryIntents: [...state.deliveryIntents.values()].map(
      deliveryIntentView,
    ),
...
export function verifyAuditExport(document) {
  ...
  if (
    ...
    !Array.isArray(document.deliveryIntents) ||
  ...
  const lastEvent = document.events.at(-1);
  if (
    document.head.tenantId !== document.tenantId ||
    document.head.tenantKind !== "SYNTHETIC" ||
    document.head.lastSequence !== (lastEvent?.sequence ?? 0) ||
    document.head.lastEventId !== (lastEvent?.eventId ?? null) ||
    document.head.lastEventHash !==
      (lastEvent?.eventHash ?? GENESIS_HASH) ||
    document.head.updatedAt !== (lastEvent?.createdAt ?? null)
  ) {
    fail("AUDIT_RECOVERY_TAMPERED", "Audit Head is invalid.");
  }

  const intentEvents = new Set();
  for (const [index, intent] of document.deliveryIntents.entries()) {
    exactKeys(
      intent,
      [
        "eventId",
      ...
      `deliveryIntents[${index}]`,
    );
    const record = eventsById.get(intent.eventId);
    if (
      !record ||
      intentEvents.has(intent.eventId) ||
      intent.retentionClass !== "AUDIT_7Y" ||
      typeof intent.legalHold !== "boolean" ||
      intent.createdAt !== record.createdAt ||
      canonicalizeAuditJson(intent.event) !==
        canonicalizeAuditJson(createAuditCloudEvent(record))
    ) {
      fail(
        "AUDIT_RECOVERY_TAMPERED",
        "Immutable DeliveryIntent is invalid.",
      );
    }
    canonicalInstant(intent.createdAt, "delivery intent createdAt");
    assertAuditMetadataOnly(intent.event);
    intentEvents.add(intent.eventId);
  }
  if (intentEvents.size !== document.events.length) {
    fail(
      "AUDIT_RECOVERY_TAMPERED",
      "Immutable DeliveryIntents are incomplete.",
    );
  }
  ...
  const body = recoveryBody({
    ...
    deliveryIntents: document.deliveryIntents,
  ...
  return Object.freeze({
    ...
    deliveryIntentCount: document.deliveryIntents.length,
...
export function createMemoryAuditEvidenceStore({
  ...
  for (const document of restoredExports) {
    ...
    for (const intent of document.deliveryIntents) {
      state.deliveryIntents.set(intent.eventId, clone(intent));
    }
  ...
  async function append(scope, input) {
    ...
    return locked(async () => {
      ...
      state.deliveryIntents.set(record.eventId, {
        eventId: record.eventId,
        event: clone(outbox.event),
...
function validateDependencies(dependencies) {
  if (
    ...
    typeof dependencies?.catalog?.resolveIdentityArtifact !== "function" ||
