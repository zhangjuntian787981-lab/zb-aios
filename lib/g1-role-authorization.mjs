...
function validateResourceInput(input, sessionKeys) {
  exactObject(input, [
    "tenantId",
    "surface",
    "resourceId",
  ...
  if (
    ...
    sessionKeys.some((key) => typeof input[key] !== "string")
  ) {
    fail("G1_ROLE_INVALID_INPUT");
  }
}
  ...
  return Object.freeze({
    ...
    async registerResource(input) {
      ...
      if (
        ...
        requiredRoleSource.tenantId !== input.tenantId
      ) {
        fail("G1_ROLE_INVALID_INPUT");
      }
      ...
      if (
        ...
        (existing.ownerPrincipalId !== owner.principalId ||
          existing.requiredRole !== requiredRoleSource.role)
      ) {
        fail("G1_ROLE_RESOURCE_POLICY_IMMUTABLE");
      }
      if (!existing) {
        const tupleKeys = [
          {
          ...
          {
            user:
              `business_role:${input.tenantId}--${requiredRoleSource.role}`,
