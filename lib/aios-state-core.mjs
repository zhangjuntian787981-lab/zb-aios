...
function verifyStoredCommandReferences(
  referenceCatalog,
  tenantId,
  command,
) {
  try {
    verifyCommandReferences(referenceCatalog, tenantId, command);
  } catch {
    fail(
      "STALE_SOURCE_REFERENCE",
      "A stored C08 reference is no longer current in its Synthetic authority.",
    );
  }
}

function verifyStoredRunBaseReferences(
  referenceCatalog,
  tenantId,
  run,
) {
  verifyStoredCommandReferences(referenceCatalog, tenantId, {
    kind: "START_RUN",
    manifest: run.baseManifest,
  });
}

function verifyStoredRunReferences(referenceCatalog, tenantId, stored) {
  verifyStoredCommandReferences(referenceCatalog, tenantId, {
    kind: "CREATE_CASE",
    goalRef: stored.case.goalRef,
  });
  verifyStoredCommandReferences(referenceCatalog, tenantId, {
    kind: "OPEN_THREAD",
    purposeRef: stored.thread.purposeRef,
  });
  for (const artifact of [
    ...
    verifyStoredCommandReferences(referenceCatalog, tenantId, {
      ...
      expectedArtifactVersion: artifact.artifactVersion - 1,
      contentRef: artifact.contentRef,
      contentSha256: artifact.contentSha256,
    });
  }
  verifyStoredRunBaseReferences(
    referenceCatalog,
    tenantId,
    stored.run,
  );
...
export function createAiosStateCore({
  ...
  async function reduceCommand({
    ...
    if (command.kind === "CREATE_CASE") {
      ...
      verifyStoredRunBaseReferences(referenceCatalog, tenantId, run);
