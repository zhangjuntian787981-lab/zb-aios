...
c04_runtime_root=$(mktemp -d /tmp/c04-keycloak-saml.XXXXXX)
c04_download_file="$c04_runtime_root/keycloak-download.tar.gz"
c04_cert_file="$c04_runtime_root/saml-cert.pem"
c04_key_file="$c04_runtime_root/saml-key.pem"
c04_log_file="$c04_runtime_root/keycloak.log"
c04_hostname="saml.c04-synthetic.example"
c04_started=0
c04_keycloak_pid=""

cleanup_c04_keycloak_saml() {
  c04_exit_status=$?
  ...
  unset C04_KC_ADMIN_PASSWORD C04_KC_USER_PASSWORD
  case "$c04_runtime_root" in
    /tmp/c04-keycloak-saml.*)
      if ! rm -rf -- "$c04_runtime_root"; then
        c04_exit_status=1
      fi
      ;;
    *)
      c04_exit_status=1
      printf '%s\n' \
        "Refusing to remove unexpected C04 SAML Keycloak path." >&2
      ;;
  esac
  if [ -e "$c04_runtime_root" ]; then
    c04_exit_status=1
    printf '%s\n' "C04 SAML temporary runtime was not removed." >&2
  else
    printf '%s\n' "C04 SAML temporary runtime removed."
  fi
  exit "$c04_exit_status"
}

trap cleanup_c04_keycloak_saml EXIT
trap 'exit 130' HUP INT TERM

for c04_binary in node curl shasum tar openssl lsof install; do
  if ! command -v "$c04_binary" >/dev/null 2>&1; then
    printf '%s\n' "C04 SAML prerequisite is missing: $c04_binary" >&2
    exit 1
  fi
done
...
c04_admin_username="c04-saml-bootstrap-admin"
c04_test_username="synthetic-saml-operator"
...
while [ "$c04_start_attempt" -lt 180 ]; do
  ...
  if ! kill -0 "$c04_keycloak_pid" >/dev/null 2>&1; then
    printf '%s\n' "C04 SAML Keycloak exited before becoming ready." >&2
    exit 1
  fi
  if curl \
    --silent \
...
if [ "$c04_ready" -ne 1 ]; then
  printf '%s\n' "C04 SAML Keycloak did not become ready within 90 seconds." >&2
  exit 1
fi
...
if ! printf '%s\n' "$c04_listeners" | grep -Fx "127.0.0.1:$c04_port" >/dev/null; then
  printf '%s\n' \
    "C04 SAML Keycloak did not expose the expected loopback listener." >&2
  exit 1
fi
if printf '%s\n' "$c04_listeners" | grep -Ev '^127\.0\.0\.1:[0-9]+$' >/dev/null; then
  printf '%s\n' "C04 SAML Keycloak exposed a non-loopback listener." >&2
  exit 1
fi

printf '%s\n' \
  "C04 Keycloak $c04_version artifact SHA-256 verified: $c04_verified_sha"
printf '%s\n' "C04 SAML Keycloak listeners verified as IPv4 loopback only."

cd "$c04_repo_root"
C04_KEYCLOAK_SAML_EPHEMERAL=1 \
...
node --test --test-concurrency=1 tests/integration/c04-keycloak-saml.test.mjs
